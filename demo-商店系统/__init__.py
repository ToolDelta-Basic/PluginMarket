...
class Shop(Plugin):
    ...
    def on_inject(self):
        ...
        self.chatbar.add_new_trigger(
            ["购买", "buy"], [], "打开商店物品购买页面", self.when_player_buy
        )
        self.chatbar.add_new_trigger(
            ["收购", "sell"], [], "打开商店物品收购页面", self.when_player_sell
        )

    def when_player_buy(self, player: Player, _):
        # player: 玩家对象
        # 由于触发词不需要参数, 因此可以舍弃这个参数 _
        # 先把所有出售的物品名整理成一个有序列表
        sells_list = list(self.sells.keys())
        if sells_list == []:
            self.game_ctrl.say_to(player.name, "§c没有可购买的物品， 已退出")
            return
        # 然后向玩家展示
        self.game_ctrl.say_to(player.name, "§6你想购买哪件商品？")
        for i, good_name in enumerate(sells_list):
            self.game_ctrl.say_to(player.name, f"{i + 1}. {good_name}")
        self.game_ctrl.say_to(player.name, "§6请输入选项序号以选择商品：")
        # 询问， 获取答复
        resp = game_utils.waitMsg(player.name)
        # 如果超时了或者玩家在中途退出游戏
        if resp is None:
            self.game_ctrl.say_to(player.name, "§c太久没有回应， 已退出")
            return
        # 回应太长
        elif len(resp) > 10:
            self.game_ctrl.say_to(player.name, "§c输入过长， 已退出")
            return
        # 如果选项不合法
        # 回应不是数字; 或者不在范围内
        ...
        # 询问需要购买多少个商品
        buy_count = utils.try_int(player.input(f"§6你需要购买多少个{good_to_buy}？"))
        if buy_count is None:
            player.show("§c输入有误， 已退出")
            return
        elif buy_count < 1:
            player.show("§c商品数量不能小于1， 已退出")
            return
        # 计算总金额并进行处理
    ...
    def when_player_sell(self, player: Player, _):
        buys_list = list(self.buys.keys())
        if buys_list == []:
            player.show("§c没有可出售的物品， 已退出")
            return
        player.show("§6你想出售哪件物品？")
        for i, good_name in enumerate(buys_list):
            player.show(f"{i + 1}. {good_name}")
        resp = player.input("§6请输入选项序号以选择：")
        if resp is None:
        ...
        if have_good < sell_count:
            player.show(f"§c你只有 {have_good} 个 {good_to_buy}， 无法出售， 已退出")
        else:
            ...
            self.game_ctrl.sendwocmd(
                f'scoreboard players add "{player.name}" {self.money_scb_name} {good_price * sell_count}'
            )
