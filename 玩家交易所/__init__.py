...
class NewPlugin(Plugin):
    ...
    def __init__(self, frame):
        ...
        self.config, _ = Config.get_plugin_config_and_version(self.name, {}, config, self.version)
        self.menu_head = self.config["交易所菜单头"].format(手续费=self.config["交易手续费"])
    ...
    def add_shop(self, player_name, player_uuid):
        ...
        while True:
            ...
            if price.isdigit():
                ...
                self.game_ctrl.say_to(player_name, f"请在30秒内将商品扔在坐标{int(pos[0])} {int(pos[1])} {int(pos[2])}处，然后发送 {self.config['提交商品']} ，输入q取消")
                while True:
                    tip = game_utils.waitMsg(player_name)
                    if tip == self.config["提交商品"]:
                        shop_list = self.game_ctrl.sendwscmd_with_resp(f"/testfor @e[x={pos[0]}, y={pos[1]}, z={pos[2]}, r = 1, type=!player]").as_dict
                        shop_list = shop_list.get("OutputMessages")[0].get("Parameters") # type: ignore
                        if shop_list == []:
                            self.game_ctrl.say_to(player_name, "§c没有找到商品。")
                            return
                        
                        self.game_ctrl.sendwocmd(f"/structure save {player_name} {pos[0]} {pos[1]} {pos[2]} {pos[0]} {pos[1]} {pos[2]}")
                        ...
                        rawtext = "{\"rawtext\": [{\"text\": \"§b" + player_name + " §f发布了商品 §b" + shop_name + "§f，价格为 §b" + str(price) + " §f。其中包含物品：§b" + f'{shop_list}' + "\"}]}"
                        self.game_ctrl.sendwocmd(f'/tellraw @a {rawtext}')
                        self.game_ctrl.sendwocmd(f"/kill @e [type =! player, x = {int(pos[0])}, y = {int(pos[1])}, z = {int(pos[2])}, r = 1]")
                        return
    ...
    def remove_shop(self, player_name, player_uuid):
        ...
        if player_uuid in self.shops:
            pos = game_utils.getPosXYZ(player_name)
            self.game_ctrl.say_to(player_name, f"§a已为您删除 §b{self.shops[player_uuid]['shopname']}")
