...
class whitelist_and_opcheck(Plugin):
    ...
    def whitelist_check(self, player_name: str | None):
        ...
        if player_uuid not in self._cfg["白名单"]["白名单玩家"]:
            self.game_ctrl.sendwocmd(f"/kick {player_name} "+ self._cfg["白名单"]["踢出提示词"])
            return
    ...
    def operation_check(self, player_name: str | None):
        ...
        if flag:
            if player_uuid not in self._cfg["管理员检测"]["管理员列表"]:
                ...
                self.game_ctrl.sendwocmd(f'/tell {player_name} {self._cfg["管理员检测"]["提示词"]}')
                return
    ...
    def on_inject(self):
        ...
        self.frame.add_console_cmd_trigger(
            ["白名单"], None, "在控制台修改白名单（需要玩家先登录一次服务器）", self.whitelist_console_set
        )
        self.frame.add_console_cmd_trigger(
            ["OP操作"], None, "在控制台修改服务器OP（需要玩家先登录一次服务器）", self.operation_console
        )
        self.auto_check_task = Utils.createThread(self.auto_check, (), "循环检测白名单和管理员")