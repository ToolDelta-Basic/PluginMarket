from tooldelta import Plugin, plugins, Print, Utils

import base64, PIL.Image, os, time

@plugins.add_plugin_as_api("SkinGetter")
class GetSkin(Plugin):
    ...
    @plugins.add_packet_listener(63)
    def on_pkt_skin(self, pkt):
        ...
        for player in pls:
            ...
            if player["Skin"]["SkinData"]:
                ...
                siz = player["Skin"]["SkinImageWidth"], player["Skin"]["SkinImageHeight"]
                img = PIL.Image.new("RGBA", siz)
                for xp in range(siz[0]):
                    for yp in range(siz[1]):
                        rs, gs, bs, alp = bskindata[xp*4+yp*siz[0]*4:xp*4+yp*siz[0]*4+4]
                        # 绘制像素点
                        img.putpixel((xp, yp), (rs, gs, bs, alp))
                img.save(f"{self.data_path}/{name}.png", bitmap_format="png")
                Print.print_with_info(f"皮肤信息: 宽={siz[0]} 高={siz[1]} 使用者={name}", info=" Skin ")
    ...
    def f_color(self, r1, g1, b1):
        weights = []
        for i, (r, g, b) in enumerate(self.cl):
            weights.append(
                (
                    (r1 - r) ** 2 +
                    (g1 - g) ** 2 +
