...
def user_bookshelf(cookie):
    ...
    return {
        ...
        "item_id_list": item_id_list,
        "read_timestamp_list": read_timestamp_list,
    }
...
def recommended_list():
    ...
    data = json.loads(requests.get(url).text)["data"]["tab_item"][0]["cell_data"][0][
        "cell_data"
    ][0]["book_data"]
    data_list = []
    for r in data:
...
def user_id_inquire(user_id):
    ...
    return {
        ...
        "read_book_time": read_book_time,
        "read_book_num": read_book_num,
        "recv_digg_num": recv_digg_num,
        "fans_num": fans_num,
        "follow_user_num": follow_user_num,
...
def reading(tone_id, item_ids):
    ...
    return {"main_url": data["main_url"], "backup_url": data["backup_url"]}


def search_books(query, passback):
    books_data = []
    url = f"https://api5-normal-lf.fqnovel.com/reading/bookapi/search/page/v/?query={query}&aid=1967&channel=0&os_version=0&device_type=0&device_platform=0&iid=466614321180296&passback={passback}&version_code=999"
    data = json.loads(requests.get(url=url).text)
    for i in data["data"]:
        book_data = {
            "book_id": i["book_data"][0]["book_id"],
            "book_name": i["book_data"][0]["book_name"],
            "author": i["book_data"][0]["author"],
            "abstract": i["book_data"][0]["abstract"],
            "category": i["book_data"][0]["category"],
            "score": i["book_data"][0]["score"],
            "sub_info": i["book_data"][0]["sub_info"],
        }
