...
def _menu_item(group: str, key: str, fallback_name: str,
    ...
    desc = item_config.get("描述")
    return (
        str(name) if isinstance(name, str) and name else fallback_name,
        str(desc) if isinstance(desc, str) else fallback_desc,
    )
...
def _show_menu(
    ...
    base_items = [
        ...
        ("管理", "管理公会成员", can_manage_members),
        ("解散", "解散公会", is_owner),
    ]
    menu_items = [
        (*_menu_item("基础功能", key, default_name, default_desc), condition)
        for key, default_desc, condition in base_items
        for default_name in (key,)
    ]
    ...
    for enabled, cmd, desc, condition in optional_menu_config:
        if enabled:
    ...
    number_prompt = _format_template(
        menu_config.get("输入数字提示模板", "§a❀ §r输入 §e[1-{count}]§r 之间的数字选择功能"),
        count=len(available_items),
    )
    name_prompt = str(menu_config.get("输入名称提示词", "§a❀ §r也可输入功能名称，输入 §cq §r退出"))
    player.show(format_panel(
        str(menu_config.get("菜单标题", "公会管理系统")),
        available_items,
        subtitle=subtitle,
...
def guild_menu_cb(self, player: Player, args: tuple):
    ...
    if guild and member and self.guild_is_frozen(guild):
        frozen_readonly_items = {
            _menu_item_name("基础功能", "列表", "列表"),
            _menu_item_name("基础功能", "查看", "查看"),
            _menu_item_name("基础功能", "成员", "成员"),
            _menu_item_name("基础功能", "日志", "日志"),
            _menu_item_name("可选功能", "排行", "排行"),
        }
...
def _apply_guild_effects_to_player(
    ...
    effects = getattr(guild, "purchased_effects", {}) or {}
    if not effects:
        return 0

    now = time.time()
    cache = getattr(self, "_effect_refresh_cache", None)
    if cache is None:
        cache = {}
        self._effect_refresh_cache = cache

    refresh_interval = getattr(Config, "EFFECT_REFRESH_INTERVAL", 3600)
    sent_count = 0

    for effect_name, raw_level in effects.items():
        if _should_stop(self):
            break
        if effect_names is not None and effect_name not in effect_names:
            continue
        ...
        try:
            self.game_ctrl.sendwocmd(
                f"effect {player_name} {effect_name} 100000 {amplifier} true"
            )
            cache[cache_key] = (amplifier, time.time())
            sent_count += 1
        except Exception as e:
            fmts.print_err(f"为玩家 {player_name} 添加效果 {effect_name} 时出错: {e}")
...
def _refresh_online_effects(
    ...
    if cache:
        for cache_key in list(cache):
            if cache_key[0] not in online_set:
                del cache[cache_key]
    ...
    for player_name in online_players:
        ...
        if guilds is not None:
            guild_id = self.guild_manager.get_cached_guild_id(player_name)
            if guild_id:
...
def guild_exp_task(self):
    ...
    while not _should_stop(self):
        if _wait_or_stopped(self, Config.EXP_UPDATE_INTERVAL):
            break
        ...
        try:
            ...
            for player_name in online_players:
                guild = self.guild_manager.get_guild_by_player(player_name)
                if guild:
                    guild_online_count[guild.guild_id] = guild_online_count.get(
                        guild.guild_id, 0) + 1
            ...
            for gid, count in guild_online_count.items():
                ...
                while next_required_exp and guild.exp >= next_required_exp:
                    guild.exp -= next_required_exp
                    level += 1
                    next_required_exp = Config.GUILD_LEVEL_EXP.get(level + 1)
            ...
            for guild_name, new_level in level_ups:
                if _should_stop(self):
                    break
                message = f"§l§a公会 §d>> §r公会 §e{guild_name}§r 升级到了 §e{new_level}§r 级！"
                self.game_ctrl.sendcmd(
                    f'/tellraw @a {{"rawtext":[{{"text":"{message}"}}]}}'
                )
        except Exception as e:
...
def update_online_task(self):
    ...
    while not _should_stop(self):
        try:
            if _wait_or_stopped(self, 300):
                break
            if not self._plugin_enabled():
                continue
            online_players = list(self.game_ctrl.allplayers)  # 这里是 List[str]
        ...
        for task in guild.tasks:
            ...
            if task.current_count >= task.target_count:
                ...
                if member:
                    ...
                    guild.exp += reward_exp
                    guild.stats.total_contribution += reward_contribution

                # 通知玩家
                if player_name in self.game_ctrl.allplayers:
                    message = (
                        f"§l§a公会任务 §d>> §r任务 '{task.name}' 已完成！"
                        f"获得 {reward_contribution} 贡献点和 {reward_exp} 公会经验"
                    )
                    self.game_ctrl.sendcmd(
                        f'/tellraw {player_name} {{"rawtext":[{{"text":"{message}"}}]}}'
                    )
...
def _paginate_display(
    ...
    while True:
        ...
        if choice is None:
            ...
            player.show(render_config_prompt("通用分页退出提示词"))
            return None
...
def custom_vault_sell(self, player: Player, args: tuple):
    ...
    player.show(
        f"§l§a公会仓库 §d>> §r自定义价格出售成功！{item_name} x{count} 已上架，价格 {custom_price} 贡献点")
...
def admin_clear_guild_data(self, player: Player, args: tuple):
    ...
    try:
        ...
        # 清理内存中的数据
        self.guild_manager.reset_runtime_state()
    ...
    if guild:
        ...
        if member:
            ...
            menu_tests = [
                ...
                ("日志", is_member),
                ("公告", is_member),
                ("仓库", Config.GUILD_FUNCTION_VAULT and is_member),
                ("管理", is_deputy_or_above),
                ("解散", is_owner),
                ("据点", Config.GUILD_FUNCTION_BASE and is_member),
