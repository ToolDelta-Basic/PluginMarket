"""Guild data persistence and cache management."""
...
class GuildManager:
    ...
    def validate_guild_data(
        ...
        if len(guild_data.members) > Config.MAX_GUILD_MEMBERS:
            ...
            errors.append(
                f"成员数量超过限制：{member_count}/{Config.MAX_GUILD_MEMBERS} "
                "Err:event.check.data.member_length_LMAX"
            )

        # 检查仓库物品
        if len(guild_data.vault_items) > Config.VAULT_INITIAL_SLOTS:
            vault_count = len(guild_data.vault_items)
            errors.append(
                f"仓库物品超过容量：{vault_count}/{Config.VAULT_INITIAL_SLOTS} "
                "Err:event.check.data.vault_items_LMAX"
            )
    ...
    def load_guilds(self, force_reload: bool = False) -> Dict[str, GuildData]:
        """Return guild data through the manager cache boundary."""
        return self._load_guilds(force_reload=force_reload)

    def rebuild_player_cache(self, guilds: Dict[str, GuildData]) -> None:
        """Rebuild the player-to-guild lookup from the provided guild map."""
        self._player_guild_cache.clear()
        for guild_id, guild in guilds.items():
            for member in guild.members:
                self._player_guild_cache[member.name] = guild_id

    def cache_player_guild(self, player_name: str, guild_id: str) -> None:
        """Record one player's guild id in the runtime cache."""
        self._player_guild_cache[player_name] = guild_id

    def get_cached_guild_id(self, player_name: str) -> Optional[str]:
        """Return the cached guild id for a player, if present."""
        return self._player_guild_cache.get(player_name)

    def reset_runtime_state(self) -> None:
        """Clear in-memory guild caches and pending save state."""
        self._cache = None
        self._player_guild_cache.clear()
    ...
    def save_guilds(self,
        ...
        try:
            ...
            self._backup_before_save(force=force)
    ...
    def _backup_before_save(self, force: bool = False) -> None:
        """按配置在写入前备份当前数据文件。"""
        safety_config = getattr(Config, "GUILD_DATA_SAFETY_CONFIG", {})
        if not safety_config.get("启用保存前备份", True):
            return
        if force and not safety_config.get("强制保存时也备份", True):
            return
        ...
        backup_path = os.path.join(backup_dir, f"公会数据文件-{timestamp}.json")
        shutil.copy2(self.file_path, backup_path)

        max_backups = int(safety_config.get("最大备份数量", 10))
        if max_backups <= 0:
            return
        ...
        for old_path in backups[max_backups:]:
            try:
                ...
                fmts.print_err(f"删除旧公会备份失败: {err}")
    ...
    def set_member_rank(
        ...
        current_guild.add_log(
            f"{player_name} 职位变更: {old_rank.display_name} -> {new_rank.display_name}")
