...
from guild_cloud_interop.models import GuildData, GuildMember, GuildTask, GuildBase, GuildRank, VaultItem  # noqa: E501
from guild_cloud_interop.config import Config
from guild_cloud_interop.prompts import render_config_prompt, render_create_guild_prompt  # noqa: E501
from guild_cloud_interop.ui import ORION_BORDER, TITLE_PREFIX, format_page_footer  # noqa: E501
            ...
            for member in guild_data.members:
                if member.name in self.game_ctrl.allplayers:
                    self.game_ctrl.sendcmd(
                        f'/tellraw {member.name} {{"rawtext":[{{"text":"§l§a公会 §d>> §r公告已更新，输入 .公会 公告 查看"}}]}}'  # noqa: E501
                    )
        except Exception as e:
...
def _handle_create_task(self, player: Player, guild: GuildData) -> bool:
    ...
    if reward_contribution < 0 or reward_contribution > max_contribution_reward:  # noqa: E501
...
def _handle_join_request_queue(self, player: Player, guild: GuildData) -> bool:
    ...
    if approved:
        ...
        player_guild_cache = getattr(
            self.guild_manager, "_player_guild_cache", None)
    ...
    if selected.player_name in self.game_ctrl.allplayers:
        ...
        self.game_ctrl.sendcmd(
            f'/tellraw {selected.player_name} {{"rawtext":[{{"text":"{message}"}}]}}'  # noqa: E501
        )
    ...
    if approved and getattr(
        ...
        for member in latest_guild.members:
            if member.name in self.game_ctrl.allplayers and member.name != selected.player_name:  # noqa: E501
                self.game_ctrl.sendcmd(
                    f'/tellraw {member.name} {{"rawtext":[{{"text":"§l§a公会 §d>> §r§e{selected.player_name}§r 加入了公会"}}]}}'  # noqa: E501
                )
...
def _handle_vault_buy(self, player: Player, guild: GuildData) -> bool:
    ...
    def formatter(i, item: VaultItem):
        ...
        time_str = datetime.fromtimestamp(
            item.timestamp).strftime("%m-%d %H:%M")
        affordable = "§a可购买" if member.contribution >= item.price else "§c贡献点不足"  # noqa: E501
        return (f"§e{i}. §f{item_name} §7x{item.count}\n"
    ...
    # 重新检查物品是否还存在
    if idx >= len(
            guild.vault_items) or guild.vault_items[idx].item_id != item.item_id:  # noqa: E501
    ...
    if seller_member and item.seller in self.game_ctrl.allplayers:
        self.game_ctrl.sendcmd(
            f'/tellraw {item.seller} {{"rawtext":[{{"text":"§l§a公会仓库 §d>> §r你的 {item_name} x{item.count} 被 {player.name} 购买了，获 {seller_income} 贡献点"}}]}}'  # noqa: E501
        )
...
def _handle_vault_logs(self, player: Player, guild: GuildData) -> bool:
    ...
    def formatter(i, log):
        ...
        return (
            ...
            f"   §7价格: §e{log.price}贡献点 §7| {participant_text} §7| 时间: §f{time_str}"  # noqa: E501
            f"{detail}\n"
        )
...
def _handle_vault_cancel(self, player: Player, guild: GuildData) -> bool:
    ...
    player.show(
        f"§l§a公会仓库 §d>> §r已撤回 §f{selected_name} §7x{
            removed_item.count}")
    if removed_item.seller != player.name and removed_item.seller in self.game_ctrl.allplayers:  # noqa: E501
        self.game_ctrl.sendcmd(
            f'/tellraw {removed_item.seller} {{"rawtext":[{{"text":"§l§a公会仓库 §d>> §r你上架的 {selected_name} x{removed_item.count} 已被 {player.name} 撤回"}}]}}'  # noqa: E501
        )
...
def _handle_dissolve_guild(self, player: Player, player_xuid: str) -> bool:
    ...
    for member in guild.members:
        if member.name in self.game_ctrl.allplayers:
            self.game_ctrl.sendcmd(
                f'/tellraw {member.name} {{"rawtext":[{{"text":"§l§a公会 §d>> §r公会 §e{guild.name}§r 已被解散"}}]}}'  # noqa: E501
            )
...
def _handle_kick_member(self, player: Player) -> bool:
    ...
    if idx is not None:
        ...
        if target.name in self.game_ctrl.allplayers:
            self.game_ctrl.sendcmd(
                f'/tellraw {target.name} {{"rawtext":[{{"text":"§l§a公会 §d>> §r你已被踢出公会"}}]}}'  # noqa: E501
            )
...
def _handle_transfer_ownership(self, player: Player) -> bool:
    ...
    if latest_new_owner.name in self.game_ctrl.allplayers:
        self.game_ctrl.sendcmd(
            f'/tellraw {latest_new_owner.name} {{"rawtext":[{{"text":"§l§a公会 §d>> §r你已成为公会会长！"}}]}}'  # noqa: E501
        )
