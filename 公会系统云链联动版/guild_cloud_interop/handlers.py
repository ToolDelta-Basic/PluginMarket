    ...
    if guild.has_permission(player.name, "audit_log"):
        ...
        if guild.audit_logs:
            for log in guild.audit_logs[-20:]:
                time_str = datetime.fromtimestamp(log.timestamp).strftime("%m-%d %H:%M")
...
def _handle_tasks(self, player: Player) -> bool:
    ...
    while True:
        latest_guild = self.guild_manager.get_guild_by_player(player.name, force_reload=True)
        if latest_guild:
            guild = latest_guild
        ...
        can_create = guild.has_permission(player.name, "task_create") or can_manage_legacy
        can_delete = guild.has_permission(player.name, "task_delete") or can_manage_legacy
        can_complete = guild.has_permission(player.name, "task_complete") or can_manage_legacy
...
def _handle_create_task(self, player: Player, guild: GuildData) -> bool:
    """创建新任务"""
    if not (guild.has_permission(player.name, "task_create") or guild.has_permission(player.name, "task_manage")):
...
def _handle_generate_auto_tasks(self, player: Player, guild: GuildData) -> bool:
    """按配置模板生成自动任务"""
    if not (guild.has_permission(player.name, "task_create") or guild.has_permission(player.name, "task_manage")):
    ...
    if max_active > 0:
        generate_count = min(generate_count, max_active - len(active_auto_tasks))
    if generate_count <= 0:
        ...
        if idx is not None:
            ...
            if confirm == "确认":
                ...
                if guild and idx < len(guild.tasks):
                    ...
                    guild.add_audit_log("task_delete", player.name, detail=removed_task.name)
                    self.guild_manager.save_guilds(guilds)
                ...
                if guild:
                    for t in guild.tasks:
                        if t.task_id == task.task_id:
                            ...
                            guild.add_audit_log("task_force_complete", player.name, detail=t.name)
                            break
...
def _notify_join_request_admins(self, guild: GuildData, applicant_name: str) -> None:
    """通知在线的申请队列处理人。"""
    if not getattr(Config, "GUILD_JOIN_REQUEST_CONFIG", {}).get("申请提交后通知在线管理员", True):
        return
    ...
    for member in guild.members:
        if (
            ...
            self.game_ctrl.sendcmd(
                f'/tellraw {member.name} {{"rawtext":[{{"text":"§l§a公会 §d>> §r§e{applicant_name} §f申请加入公会 §e{guild.name}\\n§f请在成员管理的 §a申请队列 §f中处理"}}]}}'
            )
...
def _handle_join_request_queue(self, player: Player, guild: GuildData) -> bool:
    ...
    idx = self._paginate_display(player, pending_requests, "加入申请队列", formatter, True)
    if idx is None:
        return True
    ...
    if approved:
        ...
        latest_guild.add_log(f"{selected.player_name} 加入公会 (审核人: {player.name})")
        player_guild_cache = getattr(self.guild_manager, "_player_guild_cache", None)
        if isinstance(player_guild_cache, dict):
            player_guild_cache[selected.player_name] = latest_guild.guild_id
    ...
    if approved and getattr(Config, "GUILD_JOIN_REQUEST_CONFIG", {}).get("批准后通知全体在线成员", True):
        for member in latest_guild.members:
            if member.name in self.game_ctrl.allplayers and member.name != selected.player_name:
                self.game_ctrl.sendcmd(
                    f'/tellraw {member.name} {{"rawtext":[{{"text":"§l§a公会 §d>> §r§e{selected.player_name}§r 加入了公会"}}]}}'
                )
...
def _handle_vault(self, player: Player) -> bool:
    ...
    while True:
        ...
        can_view_logs = getattr(Config, "GUILD_VAULT_CONFIG", {}).get("启用交易日志", True)
...
def _handle_vault_buy(self, player: Player, guild: GuildData) -> bool:
    ...
    if (
        item.seller == player.name
        and not getattr(Config, "GUILD_VAULT_CONFIG", {}).get("允许购买自己出售的物品", False)
    ):
    ...
    audit_ratio = float(getattr(Config, "GUILD_VAULT_CONFIG", {}).get("高价交易审计倍率", 3.0))
...
def _handle_vault_sell(self, player: Player, guild: GuildData) -> bool:
    ...
    if count <= 0 or count > item_count:
        ...
        return True
    max_sell_count = int(getattr(Config, "GUILD_VAULT_CONFIG", {}).get("单次出售最大数量", 64))
    ...
    if max_listing > 0:
        own_listing_count = sum(1 for item in guild.vault_items if item.seller == player.name)
    ...
    if vault_config.get("启用交易日志", True):
        guild.add_vault_trade_log("sell", vault_item, player.name, detail="上架出售")
...
def _handle_vault_logs(self, player: Player, guild: GuildData) -> bool:
    ...
    def formatter(i, log):
        ...
        participant_text = " §7| ".join(participants) if participants else f"操作者:{log.actor}"
...
def _handle_vault_cancel(self, player: Player, guild: GuildData) -> bool:
    ...
    idx = self._paginate_display(player, cancelable_items, "选择撤回物品", formatter, True)
    if idx is None:
        return True

    original_index, selected_item = cancelable_items[idx]
    selected_name = self._get_item_display_name(selected_item.item_id)
    player.show(f"§l§a公会仓库 §d>> §r确认撤回 §f{selected_name} §7x{selected_item.count}§r？")
    ...
    if vault_config.get("撤回后返还物品", True):
        self.game_ctrl.sendwocmd(f"give {removed_item.seller} {removed_item.item_id} {removed_item.count}")

    self.guild_manager.save_guilds(guilds)
    player.show(f"§l§a公会仓库 §d>> §r已撤回 §f{selected_name} §7x{removed_item.count}")
    if removed_item.seller != player.name and removed_item.seller in self.game_ctrl.allplayers:
        self.game_ctrl.sendcmd(
            f'/tellraw {removed_item.seller} {{"rawtext":[{{"text":"§l§a公会仓库 §d>> §r你上架的 {selected_name} x{removed_item.count} 已被 {player.name} 撤回"}}]}}'
        )
...
def _handle_create_guild(self, player: Player, player_xuid: str) -> bool:
    ...
    if self.guild_manager.create_guild(player_xuid, player.name, guild_name):
        player.show(render_create_guild_prompt("创建公会成功提示词", guild=guild_name, player=player.name))
        # 通知所有在线玩家
        ...
        payload = json.dumps({"rawtext": [{"text": announcement}]}, ensure_ascii=False)
        self.game_ctrl.sendcmd(f"/tellraw @a {payload}")
    else:
        player.show(render_create_guild_prompt("创建公会名称已存在提示词", guild=guild_name, player=player.name))
...
def _handle_list_guilds(self, player: Player) -> bool:
    ...
    page = 1
    max_page = (len(guilds) + Config.ITEMS_PER_PAGE - 1) // Config.ITEMS_PER_PAGE
    while True:
...
def _handle_return_base(self, player: Player) -> bool:
    ...
    try:
        ...
        if self.guild_is_frozen(guild):
            self.show_guild_frozen(player, guild)
            return True
        if isinstance(getattr(guild, "settings", None), dict) and guild.settings.get("base_locked", False):
...
def _handle_transfer_ownership(self, player: Player) -> bool:
    ...
    latest_guild.add_audit_log("transfer_owner", player.name, target=latest_new_owner.name)
    self.guild_manager.save_guilds(guilds)
