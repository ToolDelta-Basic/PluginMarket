...
from guild_cloud_interop.models import GuildRank, VaultItem
from guild_cloud_interop.config import Config
from guild_cloud_interop.prompts import render_create_guild_prompt
...
def quick_create_guild(self, player: Player, args: tuple):
    ...
    if not guild_name:
        player.show(render_create_guild_prompt("快捷创建缺少名称提示词"))
        return True

    if len(guild_name) < 2 or len(guild_name) > 16:
        player.show(render_create_guild_prompt("快捷创建名称长度无效提示词"))
        return True
...
def quick_join_guild(self, player: Player, args: tuple):
    ...
    if len(matched_guilds) == 1:
        ...
        if not choice or not choice.isdigit() or int(
                choice) < 1 or int(choice) > len(matched_guilds):
            player.show("§l§a公会 §d>> §r无效的选择")
            return True

        target_guild = matched_guilds[int(choice) - 1]

    if self.guild_is_frozen(target_guild):
        self.show_guild_frozen(player, target_guild)
        return True
    ...
    for member in target_guild.members:
        if member.name in self.game_ctrl.allplayers and target_guild.has_permission(  # noqa: E501
                member.name, "invite"):
    ...
    self.game_ctrl.sendcmd(
        f'/tellraw {inviter} {{"rawtext":[{{"text":"§l§a公会 §d>> §r§e{player.name} §f申请加入公会 §e{target_guild.name}\\n§f输入 §a同意 §f或 §c拒绝"}}]}}'  # noqa: E501
    )
    player.show(f"已向 {inviter} 发送加入申请")

    reply = game_utils.waitMsg(inviter, timeout=60)
    if reply == "同意":
        if self.guild_manager.add_member(
                target_guild.name, player.name, inviter):
            player.show(f"§l§a公会 §d>> §r你已加入公会 {target_guild.name}")
            self.game_ctrl.sendcmd(
                f'/tellraw {inviter} {{"rawtext":[{{"text":"§l§a公会 §d>> §r已同意申请"}}]}}'  # noqa: E501
            )
            # 通知其他在线成员
            for member in target_guild.members:
                if member.name in self.game_ctrl.allplayers and member.name != player.name:  # noqa: E501
                    self.game_ctrl.sendcmd(
                        f'/tellraw {member.name} {{"rawtext":[{{"text":"§l§a公会 §d>> §r§e{player.name}§r 加入了公会"}}]}}'  # noqa: E501
                    )
    elif reply == "拒绝":
...
def quick_donate(self, player: Player, args: tuple):
    ...
    if player.getItemCount(item_id) < amount:
        player.show(
            f"§l§a公会 §d>> §r你只有 {
                player.getItemCount(item_id)} 个{item_name}")
        return True
    ...
    if guild_id in guilds:
        ...
        if member:
            ...
            guild.stats.total_contribution += contribution
        guild.exp += exp
...
def quick_vault_sell(self, player: Player, args: tuple):
    ...
    player.show(
        f"§l§a公会仓库 §d>> §r出售成功！{item_name} x{count} 已上架，价格 {price} 贡献点")
    return True
