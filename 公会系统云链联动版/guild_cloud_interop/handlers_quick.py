...
def quick_join_guild(self, player: Player, args: tuple):  # skipcq: PY-R1000
    ...
    for member in target_guild.members:
        if member.name in self.game_ctrl.allplayers and target_guild.has_permission(
                member.name, "invite"):
    ...
    # 发送申请
    message = (
        f"§l§a公会 §d>> §r§e{player.name} "
        f"§f申请加入公会 §e{target_guild.name}\\n"
        "§f输入 §a同意 §f或 §c拒绝"
    )
    self.game_ctrl.sendcmd(
        f'/tellraw {inviter} {{"rawtext":[{{"text":"{message}"}}]}}'
    )
...
def quick_donate(self, player: Player, args: tuple):  # skipcq: PY-R1000
    ...
    if player.getItemCount(item_id) < amount:
        player.show(
            f"§l§a公会 §d>> §r你只有 {player.getItemCount(item_id)} 个{item_name}")
        return True
