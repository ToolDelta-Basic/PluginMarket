...
class GuildPlugin(Plugin):
    ...
    def __init__(self, frame: ToolDelta):
        ...
        self.config_thread = utils.createThread(config_reload_task, args=(self,), usage="公会配置热更新任务")
    ...
    def _find_guild_menu_chatbar_entry(self):
        ...
        for candidate in chatbar_triggers:
            if getattr(candidate, "usage", None) != "公会系统指令":
                continue
            if callback is not None and getattr(candidate, "func", None) is not callback:
                continue
    ...
    def on_player_join(self, player: Player):
        ...
        self.game_ctrl.sendcmd(f"/scoreboard players add {player_name} {Config.GUILD_SCOREBOARD} 0")
        self._apply_guild_effects_to_player(player.name, force=True, command_delay=0)
...
for name, func in {**handlers, **handlers_quick, **logic_functions, **guild_api_functions}.items():
