...
class MetaSpec_smpte_offset(MetaSpec):
    ...
    def encode(self, message):
        ...
        return [
            frame_rate_lookup | message.hours,
            message.minutes,
            message.seconds,
            message.frames,
            message.sub_frames,
...
class MetaSpec_key_signature(MetaSpec):
    ...
    def decode(self, message, data):
        ...
        try:
            ...
            if key < 7:
                msg = "Could not decode key with {} " "flats and mode {}".format(
                    abs(key), mode
                )
            else:
                msg = "Could not decode key with {} " "sharps and mode {}".format(
                    key, mode
                )
...
class MetaMessage(BaseMessage):
    ...
    def _setattr(self, name, value):
        ...
        if name in spec.settable_attributes:
            ...
            raise AttributeError(f"{self.type} message has no attribute {name}")
...
class UnknownMetaMessage(MetaMessage):
    def __init__(self, type_byte, data=None, time=0, type="unknown_meta", **kwargs):
        ...
        vars(self).update(
            {"type": type, "type_byte": type_byte, "data": data, "time": time}
        )
