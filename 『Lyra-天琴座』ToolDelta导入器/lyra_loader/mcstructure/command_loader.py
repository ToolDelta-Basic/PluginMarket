    ...
    def load_commands(
        ...
        try:
            for original in command_data:
                ...
                x = int(relative[0]) + start_x
                y = int(relative[1]) + start_y
                z = int(relative[2]) + start_z
                packet = dict(original)
                packet["Position"] = [x, y, z]
                self.sendaicmd(
                    f'/execute in {dim.command_id} run tp @a[name="{self.game_ctrl.bot_name}"] {x} {y} {z}'
                )
                # 仅用于确保目标区块已经加载, 不再依赖返回文本判断方块类型
