...
import json, platform, os, requests, uuid, tarfile, gzip, shutil, zipfile, socket, subprocess, io, time, threading, flask, logging
...
CQHTTP_SIGN_SERVER_LIST = [{"url": "https://qsign.wuliya.icu/8978/sign", "key": "wuliya"},
...
class GroupServerInterworking(Plugin):
    ...
    def Initialize(self) -> None:
        ...
        if self.TMPJson.read(self.ConfigPath)["配置项"]["内置签名服务端"] == True:
            ...
            if self.TMPJson.read(self.ConfigPath)["配置项"]["内置签名服务端"] == True:
                Print.print_suc("根据配置项，已启用内置签名服务端，插件将优先启动签名服务端！")
    ...
    def if_jdk_in_jdkdir(self) -> bool:
        if os.path.exists(os.path.join(self.base_SIGN_dir, "JDK-bin")) or os.path.exists(self.TMPJson.read(self.ConfigPath)["配置项"]["JDK位置"]):return True
        return False
    ...
    def if_file_run_permissions(self, file_path) -> bool:
        ...
        if os.access(file_path, os.W_OK):return True
        else:return False
    ...
    def install_jdk_17(self) -> bool:
        try:
            ...
            if not os.path.exists(os.path.join(self.base_SIGN_dir, jdk17_url.split('/')[-1])):
                if not self.if_jdk_in_jdkdir():
                    if not os.path.exists(os.path.join(self.base_SIGN_dir, urlparse(jdk17_url).path.split('/')[-1])):
                        Print.print_war("未检测到JDK，将尝试自动安装JDK，开始安装JDK...")
                        try:urlmethod.download_file_singlethreaded(jdk17_url, os.path.join(self.base_SIGN_dir, urlparse(jdk17_url).path.split('/')[-1]))
                        except Exception as e:raise ValueError(f"无法正常下载JDK压缩包文件，请自行前往[https://www.oracle.com/cn/java/technologies/downloads/#jdk17-linux]下载对应文件放置于{self.base_SIGN_dir}目录下。{e}")
            if ".zip" in jdk17_url.split('/')[-1]:
                if self.extract_archive(os.path.join(self.base_SIGN_dir, jdk17_url.split('/')[-1]), os.path.join(self.base_SIGN_dir, "jdk-17")):
                    os.remove(os.path.join(self.base_SIGN_dir, jdk17_url.split('/')[-1]))
                    old = self.TMPJson.read(self.ConfigPath)
                    old["配置项"]["JDK位置"] = os.path.join(self.base_SIGN_dir, "jdk-17")
                    self.TMPJson.write_as_tmp(self.ConfigPath, old)
                    self.reload_PathJson(self.ConfigPath)
            if os.path.exists(self.TMPJson.read(self.ConfigPath)["配置项"]["JDK位置"]):
                Print.print_suc("成功安装JDK，已自动配置 JDK位置 在插件配置中！");return True
        except Exception as e:
            Print.print_err(f"安装JDK失败，您可以手动安装(使用Linux的包管理器安装后修改插件配置文件)或将插件配置文件的JDK位置指向您已安装的地址。{e}")
            return False
        return True
    ...
    def install_sign_server(self) -> bool:
        try:
            ...
            if not os.path.exists(os.path.join(self.base_SIGN_dir, sign_server_url.split('/')[-1])):
                ...
                try:urlmethod.download_file_singlethreaded(urlmethod.githubdownloadurl_to_rawurl(fast_url[0]), os.path.join(self.base_SIGN_dir, urlparse(fast_url[0]).path.split('/')[-1]))
                except Exception as e:raise ValueError(f"无法正常下载sign-server服务端文件，请自行前往[https://github.com/CikeyQi/unidbg-fetch-qsign-shell/releases/tag/1.1.9]下载对应文件放置于{os.path.join(self.base_SIGN_dir, 'JDK-bin')}目录下。{e}")
            if ".zip" in sign_server_url.split('/')[-1]:
                if self.extract_archive(os.path.join(self.base_SIGN_dir, sign_server_url.split('/')[-1]), self.base_SIGN_dir):os.remove(os.path.join(self.base_SIGN_dir, sign_server_url.split('/')[-1]))
            os.chmod(os.path.join(self.TMPJson.read(self.ConfigPath)["配置项"]["签名服务端运行目录"], "bin", "unidbg-fetch-qsign"), 0o777)
            if not os.path.exists(self.TMPJson.read(self.ConfigPath)["配置项"]["签名服务端运行目录"]):raise ValueError("签名服务端安装失败，请检查配置文件或自行安装。")
    ...
    def use_lib_info(self) -> bool:
        try:
            shutil.copyfile(os.path.join(self.TMPJson.read(self.ConfigPath)["配置项"]["签名服务端运行目录"], "txlib", self.TMPJson.read(self.ConfigPath)["配置项"]["Sign-Server配置"]["lib版本"], "android_pad.json"), os.path.join(self.base_CQHTTP_dir, "data", "versions", "6.json"))
            shutil.copyfile(os.path.join(self.TMPJson.read(self.ConfigPath)["配置项"]["签名服务端运行目录"], "txlib", self.TMPJson.read(self.ConfigPath)["配置项"]["Sign-Server配置"]["lib版本"], "android_phone.json"), os.path.join(self.base_CQHTTP_dir, "data", "versions", "1.json"))
            return True
    ...
    def IF_SIGN_SERVER_Config(self) -> Any:
        ...
        if self.TMPJson.read(self.ConfigPath)["配置项"]["签名服务端运行目录"] == "":
            old = self.TMPJson.read(self.ConfigPath)
            old["配置项"]["签名服务端运行目录"] = os.path.join(self.base_SIGN_dir, "unidbg-fetch-qsign-1.1.9")
            self.TMPJson.write_as_tmp(self.ConfigPath, old)
            self.reload_PathJson(self.ConfigPath)
        if not os.path.exists(os.path.join(self.TMPJson.read(self.ConfigPath)["配置项"]["签名服务端运行目录"], "txlib", self.TMPJson.read(self.ConfigPath)["配置项"]["Sign-Server配置"]["lib版本"])):raise ValueError(f"签名服务端配置文件异常，签名服务端不存在版本为 [{self.TMPJson.read(self.ConfigPath)['配置项']['Sign-Server配置']['lib版本']}] 的lib！")
        if not os.path.exists(os.path.join(self.TMPJson.read(self.ConfigPath)["配置项"]["签名服务端运行目录"], "txlib", self.TMPJson.read(self.ConfigPath)["配置项"]["Sign-Server配置"]["lib版本"], "config.json")):raise ValueError(f"签名服务端配置文件异常，签名服务端在lib版本[{self.TMPJson.read(self.ConfigPath)['配置项']['Sign-Server配置']['lib版本']}] 中未找到config.json文件！")
        lib_config = self.TMPJson.read(self.SIGN_SERVER_Lib_ConfigPath)
        lib_config["server"]["host"] = self.TMPJson.read(self.ConfigPath)["配置项"]["Sign-Server配置"]["host"]
        lib_config["server"]["port"] = self.sign_server_port
        lib_config["share_token"] = self.TMPJson.read(self.ConfigPath)["配置项"]["Sign-Server配置"]["share_token"]
        lib_config["key"] = self.TMPJson.read(self.ConfigPath)["配置项"]["Sign-Server配置"]["key"]
        lib_config["auto_register"] = self.TMPJson.read(self.ConfigPath)["配置项"]["Sign-Server配置"]["auto_register"]
        ...
        try:
            if archive_path.endswith('.zip'):
                ...
                with gzip.open(archive_path, 'rb') as gz_ref:
                    with open(os.path.join(extract_dir, os.path.basename(archive_path)[:-3]), 'wb') as f_out:
    ...
    class New_CFG_CTL(object):
        ...
        def if_open_init(self) -> None:
            while self.OLD_CFG == None or self.NEW_CFG == None:
                if not self.OLD_CFG or not self.NEW_CFG:time.sleep(0.1)
                else:break
        ...
        def add_cfg_item(self, key1: str, value: Any, key2: Any | None = None) -> bool:
            try:
                if not key2 is None:
                    ...
                    return True
            except Exception as e:raise ValueError(f"向CQ-HTTP配置文件添加值时出现异常！{e}");return False
        ...
        if not self.TMPJson.read(self.ConfigPath)["配置项"]["内置签名服务端"]:
            for SERVER in CQHTTP_SIGN_SERVER_LIST:
                try:
                    ...
                    if json.loads(res.text)["code"] == 200:ON_SERVICE_SIGN_SERVER.append(SERVER)
                except:continue

            self.CQHTTPNCC.add_cfg_item("account", "sign-servers", ON_SERVICE_SIGN_SERVER)
    ...
    def SET_EVENT_HANDLE_SERVER(self) -> bool:
        try:
            ...
            self.old_cfg['servers'][0]['http']['post'][0]['url'] = f"http://{self.CQHTTPEHCore.CoreCFG['host']}:{self.CQHTTPEHCore.CoreCFG['port']}/"
    ...
    def Handle_Menu_Cmd(self, args: list[str]) -> None:
        try:
            if len(args) >= 1 :
                match args[0]:
                    case "help":Print.print_inf("§a群服互通 帮助菜单:§r\nhelp - 显示此帮助菜单\ninclude [文件路径]/[文件名] - 导入bdx文件内容到租赁服")
                    case "输入":
                        if len(args) <= 1:Print.print_err("参数长度错误，请提供正确的参数，使用(群服 help)获得帮助。");return
                        if not self.cqhttp_proc:Print.print_err("CQ-HTTP 未启动，请先启动CQ-HTTP再使用该功能!")
                    ...
                    case "停止运行签名服务器":
                        if not self.sign_server_proc:Print.print_err("SIGN-SERVER 未启动，请先启动SIGN-SERVER再使用该功能!")
                        if self.sign_server_proc.poll() != None:Print.print_err("SIGN-SERVER 未启动，请先启动SIGN-SERVER再使用该功能!")

                    case "删除插件配置文件":
                        if os.path.exists(self.ConfigPath):
                            os.remove(self.ConfigPath)
                            if not os.path.exists(self.ConfigPath):Print.print_suc("插件配置文件已成功删除，您需要重启ToolDelta重新生成配置文件，否则插件运行将会出现问题！");safe_jump(out_task=True)
                            else:Print.print_err("插件配置文件未被正确的被删除")
                        else:Print.print_err("插件配置文件不存在，无法删除！")
                    case _:Print.print_err("参数错误，请提供正确的参数，使用(群服 help)获得帮助。")
            else:
    ...
    def Handle_SIGN_SERVER_Message(self) -> Any:
        def if_sign_server_normal(interval:int = 60) -> None:
            while True:
                ...
                try:
                    rep =requests.get(f"http://{self.TMPJson.read(self.ConfigPath)['配置项']['Sign-Server配置']['host']}:{self.TMPJson.read(self.ConfigPath)['配置项']['Sign-Server配置']['端口']}", timeout=5)
                    if not rep.status_code == 200:Print.print_war('签名服务器无响应，将根据配置项 "当签名服务端无响应时自动重启": True自动重启签名服务器');self.sign_server_proc.kill()
                except:Print.print_war('签名服务器无响应，将根据配置项 "当签名服务端无响应时自动重启": True自动重启签名服务器');self.sign_server_proc.kill()
                if not self.SIGN_SERVER_RES_NUM[time.strftime('%Y-%m-%d-%H', time.localtime())] <= self.TMPJson.read(self.ConfigPath)["配置项"]["SIGN-SERVER自动重启最大限次"]:Print.print_war(f"SIGN-SERVER 在短时间内多次重启，将停止自动重启");return
                Print.print_war(f"SIGN-SERVER 进程已退出，将在 {self.TMPJson.read(self.ConfigPath)['配置项']['自动重启等待时间[S]']} 秒后尝试重启...")
                time.sleep(self.TMPJson.read(self.ConfigPath)["配置项"]["自动重启等待时间[S]"])
                self.Process_Run_SIGN_SERVER()
        ...
        def SIGN_SERVER_Handle_Message_Fuc():
            self.SIGN_SERVER_RES_NUM.setdefault(time.strftime('%Y-%m-%d-%H', time.localtime()), 0)
            self.SIGN_SERVER_RES_NUM[time.strftime('%Y-%m-%d-%H', time.localtime())] += 1
            OUTMSG: bool = self.TMPJson.read(self.ConfigPath)["配置项"]["输出签名服务器输出[可能导致刷屏]"]
            ...
            while True:
                ...
                match rec_msg:
                    case _:
                        ...
                        if "INFO ktor.application - Responding" in rec_msg:self.SIGN_SERVER_Running = True;Print.print_suc(f"§d[SIGN-SERVER]§r §a{rec_msg}§r");continue
                        if not OUTMSG:continue
                        if len(rec_msg) <=2:continue
        ...
        if self.TMPJson.read(self.ConfigPath)["配置项"]["SIGN-SERVER自动重启"]:
            Print.print_suc(f"将遵循配置项 'SIGN-SERVER自动重启': True 将在SIGN_SERVER进程退出时自动重启!")
            while True:
                if self.sign_server_proc is None or self.sign_server_proc.poll() != None and self.TMPJson.read(self.ConfigPath)["配置项"]["SIGN-SERVER自动重启"]:
                    if not self.SIGN_SERVER_RES_NUM[time.strftime('%Y-%m-%d-%H', time.localtime())] <= self.TMPJson.read(self.ConfigPath)["配置项"]["SIGN-SERVER自动重启最大限次"]:Print.print_war(f"SIGN-SERVER 在短时间内多次重启，将停止自动重启");return
                    Print.print_war(f"SIGN-SERVER 进程已退出，将在 {self.TMPJson.read(self.ConfigPath)['配置项']['自动重启等待时间[S]']} 秒后尝试重启...")
                    time.sleep(self.TMPJson.read(self.ConfigPath)["配置项"]["自动重启等待时间[S]"])
                    self.Process_Run_SIGN_SERVER()
    ...
    def Handle_CQHTTP_Message(self) -> Any:
        def CQHTTP_Handle_Message_Fuc():
            ...
            while True:
                try: rec_msg = self.cqhttp_proc.stdout.readline().decode("utf-8").strip("\n")
                    ...
                    self.CQHTTP_LOGIN_STATUS = False
                    if not self.TMPJson.read(self.ConfigPath)["配置项"]["CQHTTP自动重启"]:
                        raise ValueError(f"CQ-HTTP 进程已退出，最后得到的消息为：{self.CQHTTP_MSC.get_err_msg()} {e}")
                self.CQHTTP_MSC.append_msg(rec_msg)
                if self.cqhttp_proc.poll() != None:raise ValueError(f"CQ-HTTP 进程已退出，最后得到的消息为：{self.CQHTTP_MSC.get_err_msg()}")
                match rec_msg:
        ...
        if self.TMPJson.read(self.ConfigPath)["配置项"]["CQHTTP自动重启"]:
            Print.print_suc(f"将遵循配置项 'CQHTTP自动重启': True 将在CQHTTP进程退出时自动重启!")
            while True:
                if self.cqhttp_proc is None or self.cqhttp_proc.poll() != None and self.TMPJson.read(self.ConfigPath)["配置项"]["CQHTTP自动重启"]:
                    try:
                        if not self.CQHTTP_RES_NUM[time.strftime('%Y-%m-%d-%H', time.localtime())] <= self.TMPJson.read(self.ConfigPath)["配置项"]["CQHTTP自动重启最大限次"]:Print.print_war(f"CQ-HTTP 在短时间内多次重启，将停止自动重启");return
                    except:pass
                    Print.print_war(f"CQ-HTTP 进程已退出，将在 {self.TMPJson.read(self.ConfigPath)['配置项']['自动重启等待时间[S]']} 秒后尝试重启...")
                    time.sleep(self.TMPJson.read(self.ConfigPath)["配置项"]["自动重启等待时间[S]"])
                    self.Process_Run_CQHTTP()
    ...
    def IF_PRE_SIGN_SERVER_RUNNING(self, timeout: int = 10) -> None:
        run_time = 0
        while run_time <= timeout:
            if timeout > timeout:raise ValueError(f"SIGN-SERVER 启动超时，请自行寻求帮助 启动参数 {os.path.join(self.TMPJson.read(self.ConfigPath)['配置项']['CQHTTP运行目录'], self.TMPJson.read(self.ConfigPath)['配置项']['CQHTTP运行文件'])} -h TmpCFG.yml 工作目录 {self.TMPJson.read(self.ConfigPath)['配置项']['CQHTTP运行目录']}")
            try:
    ...
    def WAIT_SIGN_SERVER_RUNNING(self, timeout: int = 40) -> None:
        ...
        while run_time <= timeout:
            if timeout > timeout:raise ValueError(f"SIGN-SERVER 启动超时，请自行寻求帮助 启动参数 {os.path.join(self.TMPJson.read(self.ConfigPath)['配置项']['CQHTTP运行目录'], self.TMPJson.read(self.ConfigPath)['配置项']['CQHTTP运行文件'])} -h TmpCFG.yml 工作目录 {self.TMPJson.read(self.ConfigPath)['配置项']['CQHTTP运行目录']}")
            try:
                if self.SIGN_SERVER_Running == True:return
            except AttributeError:pass
            run_time += 0.5;time.sleep(0.5)
