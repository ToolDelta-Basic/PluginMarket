from tooldelta import Plugin, game_utils, plugin_entry, Chat, cfg
...
class NewPlugin(Plugin):
    ...
    def load_trust_list(self):
        ...
        return tempjson.load_and_read(
            self.trust_file, need_file_exists=False, default={}
        )
        ...
        if message.startswith(self.config["发起传送触发词"]):
            ...
            if len(parts) == 1:
                self.game_ctrl.say_to(
                    player_name, "§4§l·§a§l取消信任步骤需要指定取消信任的玩家！"
                )
            elif len(parts) == 2:
                target_name = parts[1]
        ...
        while True:
            ...
            if choice is None:
                ...
                if result_len1 == 1:
                    self.game_ctrl.say_to(
                        player_name,
                        f"§4§l·§a§l你在对方的信任清单！正在传送您至 {target_player}...",
                    )
                    self.game_ctrl.sendwocmd(f"tp {player_name} {target_player}")
                    self.game_ctrl.say_to(player_name, "§4§l·§a§l传送成功！")
                    self.game_ctrl.say_to(
                        target_player, f"§4§l·§a§l {player_name} 已传送至您身边"
                    )
                    return
                else:
                    self.game_ctrl.say_to(
                        target_player,
                        f"§4§l·§a§l {player_name} 请求传送至您。输入 'y' 接受，输入 'q' 拒绝",
                    )
                return
    ...
    def tpa_accept(self, player_name: str):
        for requester, target in list(self.tpa.items()):
            if target == player_name:
                ...
                self.game_ctrl.say_to(
                    requester, f"§4§l·§a§l您已成功传送到 {player_name}"
                )
                self.game_ctrl.say_to(
                    player_name, f"§4§l·§a§l {requester} 已传送到您这里"
                )
    ...
    def tpa_cancel(self, player_name: str):
        for requester, target in list(self.tpa.items()):
            if target == player_name:
                self.game_ctrl.say_to(
                    requester, f"§4§l·§a§l {player_name} 拒绝了您的传送请求"
                )
                del self.tpa[requester]
                return
    ...
    def teleport_player_to_target(self, player_name: str, target_name: str):
        ...
        if target_name not in online_players:
            self.game_ctrl.say_to(
                player_name, f"§4§l·§a§l玩家 {target_name} 不在线或不存在"
            )
            return
        ...
        if result_len2 == 1:
            self.game_ctrl.say_to(
                player_name,
                f"§4§l·§a§l你在对方的信任清单！正在传送您至 {target_name}...",
            )
            self.game_ctrl.sendwocmd(f"tp {player_name} {target_name}")
            self.game_ctrl.say_to(player_name, "§4§l·§a§l传送成功！")
            self.game_ctrl.say_to(
                target_name, f"§4§l·§a§l {player_name} 已传送至您身边"
            )
            return
        ...
        while True:
            response = game_utils.waitMsg(
                target_name, timeout=self.config["超时时间/s"]
            )
            if response is None:
                self.game_ctrl.say_to(player_name, "§4§l·§a§l操作超时，取消传送")
                ...
                self.game_ctrl.say_to(
                    player_name, f"§4§l·§a§l正在传送您至 {target_name}..."
                )
                self.game_ctrl.sendwocmd(f'tp "{player_name}" "{target_name}"')
                self.game_ctrl.say_to(player_name, "§4§l·§a§l传送成功！")
                self.game_ctrl.say_to(
    ...
    def add_trust(self, player_name: str, target_name: str):
        ...
        if target_name not in self.trust_list[player_name]:
            ...
            self.game_ctrl.say_to(
                player_name, f"§4§l·§a§l已将 {target_name} 添加到信任列表中"
            )
        else:
            self.game_ctrl.say_to(
                player_name, f"§4§l·§a§l {target_name} 已经在信任列表中"
            )
    ...
    def remove_trust(self, player_name: str, target_name: str):
        if player_name in self.trust_list:
            if target_name in self.trust_list[player_name]:
                ...
                self.save_trust_list()
                self.game_ctrl.say_to(
                    player_name, f"§4§l·§a§l已将 {target_name} 从信任列表中移除"
                )
            else:
                self.game_ctrl.say_to(
                    player_name, f"§4§l·§a§l {target_name} 不在信任列表中"
                )
        else:
    ...
    def trust_request(self, player_name: str):
        ...
        while True:
            self.game_ctrl.say_to(player_name, "§4§l·§a§l请输入玩家序号：")
            choice = game_utils.waitMsg(player_name, timeout=self.config["超时时间/s"])
            if choice is None:
                ...
                if target_player not in self.trust_list[player_name]:
                    ...
                    self.save_trust_list()
                    self.game_ctrl.say_to(
                        player_name, f"§4§l·§a§l已将 {target_player} 添加到信任列表中"
                    )
                else:
                    self.game_ctrl.say_to(
                        player_name, f"§4§l·§a§l{target_player} 已经在信任列表中"
                    )
                return
            else:
                self.game_ctrl.say_to(player_name, "§4§l·§a§l无效的序号，请重新输入")
