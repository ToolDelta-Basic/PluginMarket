...
class LevelCheckEnhanced(Plugin):
    ...
    def __init__(self, frame):
        ...
        # 提取配置项
        level_cfg = self.config["等级检测设置"]
        qq_cfg = self.config["QQ白名单设置"]
        ...
        try:
            ...
            if pip_api:
                ...
                try:
                    ...
                    fmts.print_suc(f"[{self.name}] websocket库安装成功，请重启ToolDelta以启用QQ功能")
    ...
    def init_whitelist_storage(self):
        ...
        try:
            # 加载白名单数据
            self.whitelist_data = tempjson.load_and_read(
                whitelist_file,
            ...
            # 加载禁用QQ列表
            self.banned_qq_list = tempjson.load_and_read(
                banned_qq_file,
                need_file_exists=False,
                default=[]
            )
            
            fmts.print_inf(f"[{self.name}] 白名单数据加载完成: {len(self.whitelist_data)}个玩家, {len(self.banned_qq_list)}个禁用QQ")
    ...
    def save_whitelist(self):
        ...
        try:
            ...
            # 保存白名单数据
            tempjson.load_and_write(whitelist_file, self.whitelist_data)
            
            # 保存禁用QQ列表
            tempjson.load_and_write(banned_qq_file, self.banned_qq_list)
    ...
    def bind_player_to_qq(self, player_name: str, qq_number: int) -> tuple[bool, str]:
        """绑定玩家到QQ号"""
        # 检查QQ是否被禁用
        ...
        if bound_player and bound_player != player_name:
            return False, f"❌ 您的QQ已绑定玩家 {bound_player}，请使用 #白名单 解绑 后重新绑定"
        
        # 检查玩家是否已被其他QQ绑定
        if player_name in self.whitelist_data and self.whitelist_data[player_name] != qq_number:
