...
class Super_AFKGobangBasic:
    ...
    def GameStart(_1P: str, _2P: str):
        rawText(_1P, "§l§7> §a五子棋游戏已开始， 退出聊天栏查看棋盘，§f输入 xz <纵坐标> <横坐标>以下子.")
        rawText(_2P, "§l§7> §a五子棋游戏已开始， 退出聊天栏查看棋盘，§f输入 xz <纵坐标> <横坐标>以下子.")
        game_control.player_title(_1P, "§e游戏开始")
        game_control.player_title(_2P, "§e游戏开始")
        game_control.player_subtitle(_1P, "§a聊天栏输入 下子 <纵坐标> <横坐标> 即可落子")
        game_control.player_subtitle(_2P, "§a聊天栏输入 下子 <纵坐标> <横坐标> 即可落子")
        ...
        while 1:
            ...
            game_control.player_actionbar(_2P, actbarText % ("§a我方下子" if this_room.isTurn(_2P) else "§6对方下子"))
            if this_room.status == "done":
                break
            if this_room.timeleft < 20:
                game_control.player_title(nowPlayer, "§c还剩 20 秒")
                game_control.player_subtitle(nowPlayer, "§6若仍然没有下子， 将会跳过你的回合")
            if this_room.timeleft <= 0:
                game_control.player_title(nowPlayer, "§c已跳过你的回合")
                game_control.player_title(_1P if _1P != nowPlayer else _2P, "§6对方超时， 现在轮到你落子")
