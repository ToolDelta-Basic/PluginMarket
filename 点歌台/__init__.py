...
class DJTable(Plugin):
    ...
    def on_inject(self):
        ...
        self.chatmenu.add_new_trigger(["点歌"], [], "点歌", self.choose_menu)
        self.chatmenu.add_new_trigger(
            ["停止当前曲目"],
            [],
            "停止当前点歌曲目",
    ...
    def search_music(self, music_name, player):
        ...
        try:
            ...
            response = requests.get(
                search_url, headers=headers, verify=False, timeout=10
            )
    ...
    def display_search_results(self, music_items, player):
        ...
        for i, item in enumerate(music_items[:10]):
            ...
            info_text = (
                item
                .xpath('.//div[@class="small text-muted"]')[0]
                .xpath("string()")
                .strip()
            ...
            file_info_text = (
                item
                .xpath('.//div[@class="row small text-muted pl-0 pl-md-9"]')[0]
                .xpath("string()")
                .strip()
            ...
            # 显示简化信息
            player.show(
                f"§e点歌§f>> §b{i + 1}. §f{title} §7[下载: {download_count}次, 评分: {rating}, 时长: {duration}]"
            )
    ...
    def download_midi(self, selected_link, selected_title):
        ...
        try:
            ...
            api_response = requests.post(
                api_url, json=payload, verify=False, timeout=10
            )
            ...
            self.print_err(f"无法下载 MIDI 文件： {e}")
            return None
    ...
    def process_midi(self, player, api_data, selected_title):
        ...
        if api_data.get("success") and api_data.get("data", {}).get("file"):
            ...
            self.midiplayer.translate_midi_to_seq_file(
                str(midi_file_path), str(midseq_path)
            )
            ...
            self.game_ctrl.say_to("@a", f"§e点歌§f>> §e{player.name}§a成功点歌:{title}")
            return True
        else:
            return False
