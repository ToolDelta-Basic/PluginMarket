...
class DJTable(Plugin):
    ...
    def choose_menu(self, player: str, args: list[str]):
        ...
        if not os.path.isfile(music_path):
            ...
            self.game_ctrl.sendwocmd(f"/scoreboard players remove {player} song_point 1")
            self.game_ctrl.say_to("@a", "§e点歌§f>> §e%s§a成功点歌:%s" % (player, music_name))
    ...
    def lookup_songs_list(self, player: str, _):
        if not self.musics_list == []:
            ...
            for i, j in enumerate(self.musics_list):
                self.game_ctrl.say_to(player, "§a%d§f. %s §7点歌: %s" % (i + 1, j[0], j))
        else:
        ...
        while 1:
            ...
            if counter > 4:
                if self.musics_list != []:
                    ...
                    self.game_ctrl.say_to("@a", "§e点歌§f>> §7开始播放§f%s§7，点歌者:§f%s" % (music_data[0], music_data[1]))
                    try:
                        self.midiplayer.playsound_at_target_sync(
                            music_data[0], "@a"
                        )
                    except SystemExit:
