        ...
        try:
            async with websockets.connect( # type: ignore
                ...
                extra_headers = {
                    ...
                    "ServerName": base64.b64encode(self.cfgs["此租赁服的公开显示名"].encode("utf-8")).decode("ascii"),
                    "ChannelName": base64.b64encode(self.cfgs["登入后自动连接到的频道大区名"].encode("utf-8")).decode("ascii"),
                    "ChannelToken": base64.b64encode(self.cfgs["频道密码"].encode("utf-8")).decode("ascii"),
                }
            ) as ws:
                ...
                if login_resp.type == "server.auth_failed":
                    Print.print_err(f"§b服服互通: 中心服务器登录失败: {login_resp.content['Reason']}")
...
class SuperLink(Plugin):
    ...
    def listen_chat(self, data: Data):
        if data.type == "chat.msg" and self.enable_trans_chat:
            self.game_ctrl.say_to("@a", Utils.simple_fmt(
                {"[服名]": data.content["Sender"], "[玩家名]": data.content["ChatName"], "[消息]": data.content["Msg"]},
