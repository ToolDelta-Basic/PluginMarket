...
def save_meta():
    ...
    with open(META_FILE, "w", encoding="utf-8") as f:
        json.dump(channels_meta, f, ensure_ascii=False, indent=4)
...
<body class="bg-slate-50 min-h-screen text-slate-800 selection:bg-blue-200 font-sans">
    <div id="app" v-cloak class="max-w-7xl mx-auto p-4 sm:p-8">
        ...
        <header class="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4">
            <div>
                <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight flex items-center">
                    <span class="text-blue-600 mr-2">✦</span> 跨服互通枢纽 <span class="ml-3 px-3 py-1 bg-blue-100 text-blue-700 text-sm rounded-full animate-pulse border border-blue-200">Active</span>
                </h1>
                <p class="text-slate-500 mt-2 ml-1 text-sm font-medium">Global Network & Authorization Gateway</p>
            </div>
            <div class="text-sm font-mono bg-white px-4 py-2 rounded-lg shadow-sm border border-slate-100 text-slate-500">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 relative overflow-hidden group hover:shadow-md transition-all">
                ...
                <div class="relative z-10">
                    <div class="text-slate-500 text-sm font-semibold mb-1 uppercase tracking-wider">活跃频道数</div>
                    <div class="text-4xl font-black text-slate-800">{{ Object.keys(channels).length }}</div>
                </div>
            </div>
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 relative overflow-hidden group hover:shadow-md transition-all">
                ...
                <div class="relative z-10">
                    <div class="text-slate-500 text-sm font-semibold mb-1 uppercase tracking-wider">在线子服数</div>
                    <div class="text-4xl font-black text-slate-800">{{ totalServers }}</div>
                </div>
            </div>
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 relative overflow-hidden group hover:shadow-md transition-all">
                ...
                <div class="relative z-10">
                    <div class="text-slate-500 text-sm font-semibold mb-1 uppercase tracking-wider">全网在线玩家</div>
                    <div class="text-4xl font-black text-slate-800">{{ totalPlayers }}</div>
                </div>
            </div>
        </div>

        <!-- 个人私密频道排行榜 -->
        <div v-if="topPrivateByServers.length > 0" class="mb-10 grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                ...
                <ul class="divide-y divide-slate-100">
                    <li v-for="(ch, idx) in topPrivateByServers" :key="ch.name" class="px-6 py-4 flex justify-between items-center hover:bg-slate-50">
                        <div class="flex items-center">
                            <span class="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold mr-3" :class="idx === 0 ? 'bg-yellow-100 text-yellow-700' : (idx === 1 ? 'bg-slate-200 text-slate-700' : (idx === 2 ? 'bg-orange-100 text-orange-800' : 'text-slate-400'))">{{ idx + 1 }}</span>
                            <span class="font-medium text-slate-700">{{ ch.name }}</span>
                        </div>
                        <span class="font-bold text-amber-600">{{ ch.serverCount }} <span class="text-xs font-normal text-slate-400">个服务器</span></span>
                    </li>
                </ul>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                ...
                <ul class="divide-y divide-slate-100">
                    <li v-for="(ch, idx) in topPrivateByPlayers" :key="ch.name" class="px-6 py-4 flex justify-between items-center hover:bg-slate-50">
                        <div class="flex items-center">
                            <span class="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold mr-3" :class="idx === 0 ? 'bg-yellow-100 text-yellow-700' : (idx === 1 ? 'bg-slate-200 text-slate-700' : (idx === 2 ? 'bg-orange-100 text-orange-800' : 'text-slate-400'))">{{ idx + 1 }}</span>
                            <span class="font-medium text-slate-700">{{ ch.name }}</span>
                        </div>
                        <span class="font-bold text-indigo-600">{{ ch.playerCount }} <span class="text-xs font-normal text-slate-400">名玩家</span></span>
                    </li>
                </ul>
            </div>
        </div>
        ...
        <!-- 频道及服务器拓扑 -->
        <div v-for="(channelData, channelName) in channels" :key="channelName" class="mb-10">
            <div class="flex items-center mb-5">
                <h2 class="text-2xl font-bold text-slate-800 flex items-center">
                    <span class="bg-slate-800 text-white w-8 h-8 rounded-lg flex items-center justify-center mr-3 text-sm">#</span>
                    {{ channelName }}
                    <span v-if="channelData.meta.type === 'private'" class="ml-3 px-2 py-0.5 bg-amber-100 text-amber-700 text-xs rounded border border-amber-200 flex items-center font-normal">
                    ...
                    <span v-else class="ml-3 px-2 py-0.5 bg-emerald-100 text-emerald-700 text-xs rounded border border-emerald-200 flex items-center font-normal">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div v-for="server in channelData.servers" :key="server.server_name" class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden hover:border-blue-200 transition-colors">
                    <div class="bg-slate-50/50 px-6 py-4 border-b border-slate-100 flex justify-between items-center">
                        <div class="font-bold text-lg text-slate-800 flex items-center">
                            <span class="w-2.5 h-2.5 rounded-full mr-3 shadow-[0_0_8px_rgba(34,197,94,0.6)]" :class="server.player_count > 0 ? 'bg-green-500' : 'bg-slate-300'"></span>
                            {{ server.server_name }}
                        </div>
                        <span class="text-xs text-slate-400 font-mono bg-slate-100 px-2 py-1 rounded">{{ server.ip }}</span>
                    </div>
                    <div class="p-6">
                        <div class="flex justify-between items-end mb-4">
                            <div class="text-sm text-slate-500 font-medium">在线玩家</div>
                            <div class="text-2xl font-bold" :class="server.player_count > 0 ? 'text-blue-600' : 'text-slate-400'">
                                {{ server.player_count }} <span class="text-sm font-medium text-slate-400">人</span>
                            </div>
                        </div>
                        
                        <div class="flex flex-wrap gap-2 max-h-32 overflow-y-auto pr-2">
                            <span v-if="server.player_count === 0" class="text-slate-400 text-sm italic py-1">💤 当前服务器无人在线</span>
                            <span v-for="player in server.players" :key="player" class="bg-blue-50 text-blue-700 text-sm font-medium px-3 py-1.5 rounded-md border border-blue-100 flex items-center">
                                <svg class="w-3.5 h-3.5 mr-1.5 opacity-70" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clip-rule="evenodd"></path></svg>
                                {{ player }}
                            </span>
                        </div>
                    </div>
        ...
        <div v-if="Object.keys(channels).length === 0" class="text-center text-slate-400 mt-20 p-10 border-2 border-dashed border-slate-200 rounded-2xl">
        ...
        <footer class="mt-16 pt-8 border-t border-slate-200 text-center text-sm text-slate-400 pb-8">
        ...
        createApp({
            setup() {
                ...
                const totalServers = computed(() => {
                    let count = 0
                    for (const key in channels.value) count += channels.value[key].servers.length
                    return count
                })
                ...
                const totalPlayers = computed(() => {
                    ...
                    for (const key in channels.value) {
                        channels.value[key].servers.forEach(s => count += s.player_count)
                    }
                    return count
                })
                ...
                const topPrivateByServers = computed(() => {
                    ...
                    return privates.sort((a, b) => b.serverCount - a.serverCount).slice(0, 5);
                });
                ...
                const topPrivateByPlayers = computed(() => {
                    const privates = Object.entries(channels.value)
                        ...
                        .map(([name, data]) => ({
                            name,
                            playerCount: data.servers.reduce((sum, s) => sum + s.player_count, 0)
                        }));
                    return privates.sort((a, b) => b.playerCount - a.playerCount).slice(0, 5);
                });
                ...
                const fetchData = async () => {
                    try {
                        ...
                        const now = new Date()
                        lastUpdate.value = now.toLocaleTimeString('en-US', { hour12: false }) + '.' + now.getMilliseconds().toString().padStart(3, '0').slice(0,1)
                    } catch (e) {
...
async def handle_client(websocket, _path):  # skipcq: PY-R1000
    ...
    try:
        ...
        if websocket in clients:
            ...
            # 已移除旧版的“频道为空自动销毁密码”逻辑
            # 现在私人频道的密码将被永久保留，绝对防抢注！
