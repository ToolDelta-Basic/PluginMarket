...
class BattleEye(Plugin): #插件主类
    ...
    def __init__(self, frame):
        ...
        config, config_version = cfg.get_plugin_config_and_version( #调用配置
        ...
        if self.speak_speed_limit == True or self.repeat_message_limit == True: #创建异步计时器，用于刷新“检测发言频率”和“检测重复刷屏”的缓存
            self.data = {}
    ...
    def blacklist_word_detect(self,message,player): #黑名单词检测函数封装
        if self.Testfor_blacklist_word == True and player not in self.whitelist:
    ...
    def message_length_detect(self,message,player): #发言字数检测函数封装
        if self.message_length_limit == True and player not in self.whitelist:
    ...
    def speak_speed_detect(self,player): #发言频率检测函数封装
        if self.speak_speed_limit == True and player not in self.whitelist:
            if len(self.data[player]) > self.max_speak_count:
    ...
    def repeat_message_detect(self,player): #重复消息刷屏检测函数封装
        if self.repeat_message_limit == True and player not in self.whitelist:
            counts = {}
    ...
    def on_PlayerList(self, packet):
        ...
        if packet["ActionType"] == 0:
            ...
            if Username not in self.whitelist:
                ...
                if self.is_detect_bot == True and (PremiumSkin == False or Trusted == False or CapeID == None):
                ...
                if self.is_level_limit == True and GrowthLevels < self.server_level:
                ...
                if self.is_detect_netease_banned_word == True:
                    try:
                ...
                if self.is_detect_self_banned_word == True:
        ...
        if packet["TextType"] == 10 and packet["XUID"] != "": #"TextType"=10:监听到命令执行反馈
                ...
                if commands_type == "commands.message.display.outgoing" and original_player not in self.whitelist: #"commands.message.display.outgoing":监听到游戏内私聊(tell,msg,w命令)
                    for i in rawtext_list:
                        if "with" in i:
                            ...
                            break
                    if self.ban_private_chat == True:
                        if self.allow_chat_with_bot == True:
            ...
            try:
                ...
                if message.startswith("*") and sourcename == "": #判断为me命令
                    ...
                    if self.ban_me_command == True and player not in self.whitelist:
