...
class BattleEye(Plugin):  # 插件主类
    ...
    def __init__(self, frame):
        ...
        self.ListenPacket(
            PacketIDS.IDPlayerList, lambda pk: self.on_PlayerList(pk) or True
        )  # 监听PlayerList数据包
        self.ListenPacket(PacketIDS.IDText, self.on_Text)  # 监听Text数据包

        if (
            self.speak_speed_limit or self.repeat_message_limit
    ...
    def on_active(self):
        self.bot_name = (
            self.frame.get_players().getBotInfo().name
        )
        fmts.print_inf(
        ...
        if packet["ActionType"] == 0:
            ...
            if Username not in self.whitelist:
                ...
                if self.is_level_limit and GrowthLevels < self.server_level:
                    fmts.print_inf(
                        f"§c发现 {Username} 等级低于服务器准入等级，正在踢出"
                    )
                    self.game_ctrl.sendwocmd(
                        f'/kick "{Username}" 本服准入等级为{self.server_level}级，您的等级过低，请加油升级噢！'
                    )
                    fmts.print_inf(
                        f"§a发现 {Username} 等级低于服务器等级，已被踢出游戏"
                    )
                    ...
                    for i in range(n):
                        for j in range(i + 1, n + 1):
                            if Username[i:j] in self.banned_word_set:
                                fmts.print_inf(
                                    f"§c发现 {Username} 名称为自定义违禁词({Username[i:j]})，正在踢出"
                                )
                                self.game_ctrl.sendwocmd(
                                    f'/kick "{Username}" 您必须通过 Microsoft 服务身份验证。'
                                )
                                fmts.print_inf(
                                    f"§a发现 {Username} 名称为自定义违禁词({Username[i:j]})，已被踢出游戏"
                                )

    def on_Text(self, packet):
        if (
            packet["TextType"] == 10 and packet["XUID"] != ""
            ...
            try:
                ...
                print(f"在解析私聊数据包或某些命令数据包时出现错误: {error}")
            ...
            try:
                ...
                print(f"在解析发言数据包或me命令时出现错误 {error}")
