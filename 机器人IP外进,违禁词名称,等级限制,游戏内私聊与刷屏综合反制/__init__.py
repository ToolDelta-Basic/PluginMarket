...
class BattleEye(Plugin):
            ...
            for i in range(n):
                for j in range(i + 1, n + 1):
                    if message[i:j] in blacklist_word_set:
                        ...
                        self.ban_player_first_time(player, self.ban_time_testfor_blacklist_word, f"您发送的文本触发了黑名单词({message[i:j]})")
    ...
    def message_length_detect(self, message, player):
        if self.message_length_limit and len(message) > self.max_speak_length and player not in self.whitelist:
            ...
            self.ban_player_first_time(player, self.ban_time_message_length_limit, f"您发送的文本长度超过{self.max_speak_length}字符限制")
    ...
    def speak_speed_detect(self, player):
        if self.speak_speed_limit and len(self.data[player]) > self.max_speak_count and player not in self.whitelist:
            ...
            self.ban_player_first_time(player, self.ban_time_speak_speed_limit, f"您发送文本速度超过限制({self.max_speak_count}条/{self.speak_detection_cycle}秒)")
        ...
        if self.repeat_message_limit and player not in self.whitelist:
            ...
            for _, v in counts.items():
                if v > self.max_repeat_count:
                    ...
                    self.ban_player_first_time(player, self.ban_time_repeat_message_limit, f"您连续发送重复文本超出限制({self.max_repeat_count}条/{self.speak_detection_cycle}秒)")
                    break
    ...
    def ban_player_first_time(self, player, ban_time, ban_reason):
        ...
        if pre_ban_timestamp == "Forever":
            ...
            elif type(ban_time) == int and ban_time > 0:
                timestamp_until = pre_ban_timestamp + ban_time
                data_until = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(timestamp_until))
                tempjson.load_and_write(
                    path,
                    {
                    "xuid": xuid,
            ...
            # type(ban_time) == str:如果输入的是字符串，对字符串进行解析，判断是否合法，提取时间信息
                ...
                for value_str, unit in matches_time_units:
                    ...
                    try:
                        ...
                        print(f"执行封禁操作失败：封禁时间中存在无效的数值：{value_str} detail: {str(error)}")
                        return
    ...
    def ban_player_when_PlayerList(self, player):
        ...
        try:
            ...
            if type(ban_until_timestamp) == int: 
        ...
        if packet["ActionType"] == 0:
            ...
            if Username not in self.whitelist:
                ...
                if self.is_detect_bot and (PremiumSkin is False or Trusted is False or CapeID is None):
                    ...
                    self.ban_player_first_time(Username, self.ban_time_detect_bot, "您必须通过 Microsoft 服务身份验证。")
                ...
                if self.is_level_limit and GrowthLevels < self.server_level:
                    ...
                    self.ban_player_first_time(Username, self.ban_time_level_limit, f"您的等级低于服务器准入等级({self.server_level}级)")
                ...
                if self.is_detect_netease_banned_word:
                    try:
                        ...
                        self.ban_player_first_time(Username, self.ban_time_detect_netease_banned_word, "您的名称为网易屏蔽词")
                    ...
                    for i in range(n):
                        for j in range(i + 1, n + 1):
                            if Username[i:j] in self.banned_word_set:
                                ...
                                self.ban_player_first_time(Username, self.ban_time_detect_self_banned_word, f"您的名称为本服自定义违禁词({Username[i:j]})")
                    ...
                    if self.ban_private_chat:
                        if self.allow_chat_with_bot:
                            if target_player != self.bot_name:
                                ...
                                self.ban_player_first_time(original_player, self.ban_time_private_chat, "您尝试发送私聊(tell,msg,w命令)")
                        else:
                            fmts.print_inf(f"§c发现 {original_player} 尝试发送私聊(tell,msg,w命令)，正在踢出")
                            self.game_ctrl.sendwocmd(f"/kick \"{original_player}\" 禁止发送私聊(tell,msg,w命令)！")
                            fmts.print_inf(f"§a发现 {original_player} 尝试发送私聊(tell,msg,w命令)，已被踢出游戏")
            ...
            try:
                ...
                if message.startswith("*") and sourcename == "":
                    ...
                    if self.ban_me_command and player not in self.whitelist:
                        ...
                        self.ban_player_first_time(player, self.ban_time_me_command, "您尝试发送me命令")
