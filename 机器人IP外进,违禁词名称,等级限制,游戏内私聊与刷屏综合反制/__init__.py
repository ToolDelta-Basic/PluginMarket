...
class BattleEye(Plugin):
    ...
    def __init__(self, frame):
        ...
        self.detect_netease_banned_word_timeout = config["--网易屏蔽词名称检测等待时间(秒)"]
        self.is_detect_self_banned_word = config["是否启用自定义违禁词名称反制"]
        self.banned_word_list = config["名称违禁词列表"]
        self.is_check_player_info = config["是否检查玩家信息与网易启动器是否一致(可用于反制外挂篡改游戏内等级，注意：本API不稳定，当大量玩家同时进入游戏时可能出现404或超时)"]
        self.is_ban_player_if_cannot_search = config["--如果在网易启动器无法搜索到该玩家，是否踢出游戏(可能的原因：“本API调用过快”、“玩家为机器人”、“玩家名称为网易屏蔽词”、“玩家在10分钟内改过名字，但数据库暂未更新)"]
        self.is_ban_player_if_different_level = config["--如果在网易启动器搜到的玩家等级与游戏内等级不同(说明遭到外挂篡改)，是否踢出游戏"]
        self.check_player_info_api_timeout = config["--网易启动器检查API响应等待时间(秒)"]
        self.check_player_info_api_try_time = config["--网易启动器检查API可尝试次数(最后一次尝试依然搜索失败即踢出)"]
        ...
        self.ban_time_if_different_level = config["--封禁时间_网易启动器搜到的玩家等级与游戏内等级不同"]
    ...
    def on_Text(self, packet):
        ...
        if packet["TextType"] == 10 and packet["XUID"] != "":
            try:
                ...
                if commands_type == "commands.message.display.outgoing" and original_player not in self.whitelist:
                    for i in rawtext_list:
                        if "with" in i:
                    ...
                    self.ban_time_format_and_first_execute(original_player, self.ban_time_private_chat, "您尝试发送私聊(tell,msg,w命令)")
        ...
        # "TextType"=1:监听到常规发言或me命令
            ...
            try:
                ...
                if message.startswith("*") and sourcename == "":
                    ...
                    self.ban_time_format_and_first_execute(player, self.ban_time_me_command, "您尝试发送me命令")
                # 判断为常规发言
                elif sourcename != "":
    ...
    def ban_bot(self, Username, PremiumSkin, Trusted, CapeID, packet):
        if self.is_detect_bot and (PremiumSkin is False or Trusted is False or CapeID is None):
            ...
            self.ban_time_format_and_first_execute(Username, self.ban_time_detect_bot, "您必须通过 Microsoft 服务身份验证。")
    ...
    def ban_player_level_too_low(self, Username, GrowthLevels):
        if self.is_level_limit and GrowthLevels < self.server_level:
            fmts.print_inf(f"§c发现 {Username} 等级低于服务器准入等级({self.server_level}级)，正在踢出")
            self.game_ctrl.sendwocmd(f"/kick \"{Username}\" 本服准入等级为{self.server_level}级，您的等级过低，请加油升级噢！")
            fmts.print_inf(f"§a发现 {Username} 等级低于服务器准入等级({self.server_level}级)，已被踢出游戏")
            self.ban_time_format_and_first_execute(Username, self.ban_time_level_limit, f"您的等级低于服务器准入等级({self.server_level}级)")
    ...
    def ban_player_with_netease_banned_word(self, Username):
        if self.is_detect_netease_banned_word:
            try:
                self.game_ctrl.sendcmd(f"/testfor \"{Username}\"", True, self.detect_netease_banned_word_timeout)
            except TimeoutError:
                fmts.print_inf(f"§c发现 {Username} 名称为网易屏蔽词，正在踢出")
                self.game_ctrl.sendwocmd(f"/kick \"{Username}\" 您必须通过 Microsoft 服务身份验证。")
                fmts.print_inf(f"§a发现 {Username} 名称为网易屏蔽词，已被踢出游戏")
                self.ban_time_format_and_first_execute(Username, self.ban_time_detect_netease_banned_word, "您的名称为网易屏蔽词")
            ...
            for i in range(n):
                for j in range(i + 1, n + 1):
                    if Username[i:j] in self.banned_word_set:
                        fmts.print_inf(f"§c发现 {Username} 名称为本服自定义违禁词({Username[i:j]})，正在踢出")
                        self.game_ctrl.sendwocmd(f"/kick \"{Username}\" 您必须通过 Microsoft 服务身份验证。")
                        fmts.print_inf(f"§a发现 {Username} 名称为本服自定义违禁词({Username[i:j]})，已被踢出游戏")
                        self.ban_time_format_and_first_execute(Username, self.ban_time_detect_self_banned_word, f"您的名称为本服自定义违禁词({Username[i:j]})")
    ...
    def check_player_info(self, Username, GrowthLevels, packet):
        if self.is_check_player_info:
            ...
            while True:
                try:
                    userdata_from_netease = requests.post(url, json=payload, headers=headers, timeout=self.check_player_info_api_timeout)
                    try_time += 1

                    if userdata_from_netease.status_code == 404:
                        if try_time >= self.check_player_info_api_try_time:
                            fmts.print_inf(f"§c在网易启动器搜索玩家 {Username} 失败，原因：状态码[{userdata_from_netease.status_code}]，当前尝试次数{try_time}/{self.check_player_info_api_try_time}，这是最后一次尝试")
                            if self.is_ban_player_if_cannot_search:
                                fmts.print_inf(f"§c由于我们无法在网易启动器搜索到玩家 {Username} ，正在踢出该玩家，这可能是由于“玩家为机器人”、“玩家名称为网易屏蔽词”、“玩家在10分钟内改过名字，但数据库暂未更新”等原因导致的")
                                self.game_ctrl.sendwocmd(f"/kick \"{Username}\" 您必须通过 Microsoft 服务身份验证。")
                                fmts.print_inf(f"§a由于我们无法在网易启动器搜索到玩家 {Username} ，该玩家已被踢出游戏")
                                self.ban_time_format_and_first_execute(Username, self.ban_time_if_cannot_search, "您必须通过 Microsoft 服务身份验证。")
                            break
                        else:
                            try_api_sleep_time = random.randint(15*try_time, 30*try_time)
                            fmts.print_inf(f"§c在网易启动器搜索玩家 {Username} 失败，原因：状态码[{userdata_from_netease.status_code}]，当前尝试次数{try_time}/{self.check_player_info_api_try_time}，将在{try_api_sleep_time}秒后再次尝试搜索")
                        ...
                        for i in userdata_from_netease["data"]:
                            if i["nickname"] == Username:
                                fmts.print_inf(f"§a成功在网易启动器搜索到玩家 {Username} ，其启动器等级为{i['lv']}，游戏内等级为{GrowthLevels}")
                                if self.is_ban_player_if_different_level and i["lv"] != GrowthLevels:
                                    fmts.print_inf(f"§c由于玩家 {Username} 的启动器等级和游戏内等级不匹配，正在踢出")
                                    fmts.print_war(f"该玩家可能通过外挂篡改游戏内等级: {packet}")
                                    self.game_ctrl.sendwocmd(f"/kick \"{Username}\" 您必须通过 Microsoft 服务身份验证。")
                                    fmts.print_inf(f"§a由于玩家 {Username} 的启动器等级和游戏内等级不匹配，已被踢出游戏")
                                    self.ban_time_format_and_first_execute(Username, self.ban_time_if_different_level, "您必须通过 Microsoft 服务身份验证。")
                        break
                    ...
                    try_time += 1
                    if try_time >= self.check_player_info_api_try_time:
                        fmts.print_inf(f"§c在网易启动器搜索玩家 {Username} 失败，原因：请求网易启动器玩家信息超时：{timeout_error}，当前尝试次数{try_time}/{self.check_player_info_api_try_time}，这是最后一次尝试")
                        if self.is_ban_player_if_cannot_search:
                            fmts.print_inf(f"§c由于我们无法在网易启动器搜索到玩家 {Username} ，正在踢出该玩家，这可能是由于“玩家为机器人”、“玩家名称为网易屏蔽词”、“玩家在10分钟内改过名字，但数据库暂未更新”等原因导致的")
                            self.game_ctrl.sendwocmd(f"/kick \"{Username}\" 您必须通过 Microsoft 服务身份验证。")
                            fmts.print_inf(f"§a由于我们无法在网易启动器搜索到玩家 {Username} ，该玩家已被踢出游戏")
                        ...
                        try_api_sleep_time = random.randint(15*try_time, 30*try_time)
                        fmts.print_inf(f"§c在网易启动器搜索玩家 {Username} 失败，原因：请求网易启动器玩家信息超时：{timeout_error}，当前尝试次数{try_time}/{self.check_player_info_api_try_time}，将在{try_api_sleep_time}秒后再次尝试搜索")
                        time.sleep(try_api_sleep_time)
                    ...
                    print(f"请求网易启动器玩家信息失败(API请求失败或玩家数据json解析失败)：{error}")
                    break
            ...
            for i in range(n):
                for j in range(i + 1, n + 1):
                    if message[i:j] in blacklist_word_set:
                        ...
                        self.ban_time_format_and_first_execute(player, self.ban_time_testfor_blacklist_word, f"您发送的文本触发了黑名单词({message[i:j]})")
    ...
    def message_length_detect(self, message, player):
        if self.message_length_limit and len(message) > self.max_speak_length and player not in self.whitelist:
            ...
            self.ban_time_format_and_first_execute(player, self.ban_time_message_length_limit, f"您发送的文本长度超过{self.max_speak_length}字符限制")
    ...
    def speak_speed_detect(self, player):
        if self.speak_speed_limit and len(self.data[player]) > self.max_speak_count and player not in self.whitelist:
            ...
            self.ban_time_format_and_first_execute(player, self.ban_time_speak_speed_limit, f"您发送文本速度超过限制({self.max_speak_count}条/{self.speak_detection_cycle}秒)")
        ...
        if self.repeat_message_limit and player not in self.whitelist:
            ...
            for _, v in counts.items():
                if v > self.max_repeat_count:
                    ...
                    self.ban_time_format_and_first_execute(player, self.ban_time_repeat_message_limit, f"您连续发送重复文本超出限制({self.max_repeat_count}条/{self.speak_detection_cycle}秒)")
    ...
    def ban_time_format_and_first_execute(self, player, ban_time, ban_reason):
        ...
        if ban_time != 0:
            ...
            # type(ban_time) is int and ban_time > 0:封禁玩家对应时间(单位:秒)
                ...
                date_until = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(timestamp_until))
                tempjson.load_and_write(
                    path,
                    {
                        "xuid": xuid,
                ...
                if not matches_time_units:
                    print(f"警告：封禁时间({ban_time})中无法匹配到任何时间单位，合法的时间单位为(年|月|日|时|分|秒)")
                    return

                ban_time_after_matched = "".join(f"{value}{unit}" for value, unit in matches_time_units)
                ...
                for value_str, unit in matches_time_units:
                    ...
                    try:
                        ...
                        print(f"警告：封禁时间({ban_time})中存在无效的数值：{value_str} detail: {str(error)}")
                        return
    ...
    def ban_player_when_PlayerList(self, player):
        ...
        try:
            ...
            if type(ban_until_timestamp) is int:
                ban_until_date = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(ban_until_timestamp))
                timestamp_now = int(time.time())
                if ban_until_timestamp > timestamp_now:
                    fmts.print_inf(f"§c发现 {player} 被封禁，正在踢出，其解封时间为：{ban_until_date}")
                    self.game_ctrl.sendwocmd(f"/kick \"{player}\" 由于{ban_reason}，您被系统封禁至：{ban_until_date}")
                    fmts.print_inf(f"§a发现 {player} 被封禁，已被踢出游戏")
                else:
                    os.remove(path)
            elif ban_until_timestamp == "Forever":
                fmts.print_inf(f"§c发现 {player} 被封禁，正在踢出，该玩家为永久封禁")
                self.game_ctrl.sendwocmd(f"/kick \"{player}\" 由于{ban_reason}，您被系统封禁至：Forever")
                fmts.print_inf(f"§a发现 {player} 被封禁，已被踢出游戏")
