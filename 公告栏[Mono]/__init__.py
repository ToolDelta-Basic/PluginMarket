...
class BetterAnnounce(Plugin):
    ...
    def on_def(self):
        ...
        self.刷新时间 = 1
        self.title = "公告栏"
        self.tpscalc = plugins.get_plugin_api("tps计算器", (0, 0, 1), True)
    ...
    def flush_gg(self):
        ...
        res=self.game_ctrl.sendwscmd("/scoreboard objectives list",waitForResp=True,timeout=5)
        if res:
            ...
            if not have_scoreboard:
                ...
                self.game_ctrl.sendwocmd(f"/scoreboard objectives add 公告 dummy {self.title}")
        else:
            raise KeyError("获取计分板列表失败")
        while True:
            if repeat_times >5:
                self.print("重试次数过多,可能无法正常显示公告栏,请确认租赁是否服流畅,这可能导致命令发送失败")
                raise TimeoutError("公告栏重试失败次数过多.")
            self.print("§e尝试删除重建公告栏[1/3]")
            res=self.game_ctrl.sendwscmd("/scoreboard objectives remove 公告",waitForResp=True,timeout=3)
            if res:
            ...
            res=self.game_ctrl.sendwscmd(
                f"/scoreboard objectives add 公告 dummy {self.title}",timeout=3,waitForResp=True
            )
            if res:
                if res.SuccessCount == 0:
                    repeat_times+=1
            ...
            res=self.game_ctrl.sendwscmd("/scoreboard objectives setdisplay sidebar 公告",timeout=3,waitForResp=True)
            if res:
                if res.SuccessCount == 0:
                    repeat_times+=1
            ...
            for text in self.ads_texts_bak:
                ...
                if self.on_first_run:
                    ...
                    while True:
                        ...
                        res=self.game_ctrl.sendwscmd(f'/scoreboard players set "{text}" 公告 {scb_score}',waitForResp=True,timeout=3)
                        if res:
                            if res.SuccessCount == 0:
                                self.print(f"§c设置公告栏内容['{text}']失败§f,将在§e{repeat_times*3}s§f后重试")
                                time.sleep(repeat_times*3)
                                continue
                        else:
                            self.print(f"§c设置公告栏内容['{text}']失败§f,将在§e{repeat_times*3}s§f后重试")
    ...
    @plugins.add_packet_listener([packets.IDSetScore])
    def on_setscore(self,packet:dict):
        if packet.get("ActionType",None) != None:
            if packet["ActionType"] == 1:
                for i in packet["Entries"]:
                    if i["ObjectiveName"] == "公告":
                        self.record_del_and_create["create"][i["EntryID"]] = i["DisplayName"]
            else:
                for i in packet["Entries"]:
