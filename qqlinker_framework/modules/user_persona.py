...
class UserPersonaService:
    ...
    def set_persona(self, user_id: int, persona: str):
        _logger.debug("[Persona] 写入人设 user_id=%d -> '%s'", user_id, persona)
...
class UserPersonaModule(Module):
    ...
    async def _cmd_set(self, ctx):
        ...
        try:
            ...
            _logger.debug("[Persona] 设置令牌 user_id=%d token=%s", ctx.user_id, token)
            ...
            await ctx.reply(f"已设定你的人设：{persona}（但 AI 核心未就绪，角色可能延迟生效）")
