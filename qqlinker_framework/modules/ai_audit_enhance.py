...
class AuditKnowledgeStore:
    """审计知识存储，支持 L1 案例、L2 元知识、L3 法则，具备归纳能力。"""

    def __init__(self, data_dir: str):
        self._case_file = os.path.join(data_dir, "cases.jsonl")   # L1
        self._meta_file = os.path.join(data_dir, "meta_knowledge.json")  # L2 & L3
    ...
    async def get_active_meta(self, level: str = "L2") -> List[Dict]:
        """获取当前激活的元知识（L2 或 L3）。"""
        return [m for m in self._meta if m.get("level") == level and m.get("status") == "active"]
    ...
    def _build_induction_prompt(self, cases: List[dict]) -> str:
        """构造归纳提示词。"""
        cases_text = "\n".join(
            [f"- 用户消息: {c['user_msg'][:100]} ... \n  AI回复被标记: {c.get('violation', '')}" for c in cases[-50:]]
        )
        return (
            ...
            "输出JSON数组，每条元知识包含:\n"
            "{\"level\": \"L2\", \"content\": \"...\", \"trigger_scenario\": \"...\", \"core_correction\": \"...\"}"
        )
...
class AIAuditEnhanceModule(Module):
    ...
    async def on_init(self):
        ...
        self.listen("AIPrePromptReflectionEvent", self._on_pre_reflection, priority=10)
        self.listen("AIPostResponseReflectionEvent", self._on_post_reflection, priority=10)
    ...
    async def _on_pre_reflection(self, event: AIPrePromptReflectionEvent):
        ...
        if not self._pre_reflection_enabled:
            return
        # 简单模拟：检查是否包含敏感词（可替换为实际审计逻辑）
        keywords = ["攻击", "破解", "外挂"]
        msg = event.message
        found = [kw for kw in keywords if kw in msg]
        if found:
            event.supplement = f"【风险提醒】消息中包含关键词：{', '.join(found)}，回复时请注意避免提供违规帮助。"
    ...
    async def _on_post_reflection(self, event: AIPostResponseReflectionEvent):
        ...
        sensitive = ["外挂", "破解教程"]
        reply = event.reply
        violations = [kw for kw in sensitive if kw in reply]
        if violations:
            warning = f"【违规通知】你的回复中包含了 {', '.join(violations)}，违反了安全规则。"
            ...
            if self._pending_count >= self._induction_threshold:
                self._pending_count = 0
                # 由于模块没有直接访问 LLM 客户端，这里只记录提示，实际归纳需要管理员手动触发或通过工具调用
                _logger.info("已达到归纳阈值，建议管理员执行 '.归纳知识' 命令")
                # 可在未来由定时任务或命令触发
