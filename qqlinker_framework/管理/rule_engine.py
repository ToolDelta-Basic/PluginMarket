...
from qqlinker_framework.core.kernel.services import UID_NOBODY
...
class RuleService:
    ...
    def match_rules(self, text: str, group_id: int) -> List[tuple]:
        ...
        results = []
        rules_path = os.path.join(self._base_path, _RULES_PREFIX, f'{group_id}.json')
...
class RuleEngineModule(Module):
    ...
    async def on_init(self):
        # on_init 时 data_dir 已就绪，更新 base_path
    ...
    async def _cmd_create(self, ctx):
        ...
        try:
            ...
            pass
        await ctx.reply("📝 请输入规则名称（或输入 取消 退出，5分钟超时）：")
    ...
    async def _on_rule_input(self, event):
        ...
        try:
            ...
            matches = self._rule_service.match_rules(text, group_id)
    ...
    async def _handle_create_step(self, event, session: dict, text: str, uid: str):
        ...
        if step == "actions":
            if text == "完成":
                await next_step("skip_on_fail")
                await self.message.send_group(gid,
                    "动作链中某条失败时，是否继续执行后续动作？(是/否):")
                return
            data["动作链"].append(text)
            return  # 继续收集动作

        if step == "skip_on_fail":
            data["失败跳过"] = text.strip().lower() in ("是", "yes", "y", "1", "true")
            data["启用"] = True
            ...
            lines = [
                ...
                f"  动作: {len(data['动作链'])} 条",
                f"  失败: {'跳过继续' if data['失败跳过'] else '中断'}",
            ]
            await self.message.send_group(gid, "\n".join(lines))
    ...
    def _route_command(self, cmd_text: str, user_id: int, group_id: int):
        ...
        try:
            ...
            return
        from qqlinker_framework.core.kernel.events import GroupMessageEvent  # noqa: F811
