...
class CommandSenderMonitor(Plugin):
    ...
    def special_monitor_sendcmd(self, cmd: str, waitForResp: bool = False, timeout: float = 30):
        if self.instant_monitor:
            Print.print_with_info(f"发送§a普通§r 指令{'<有返回>' if waitForResp else ''}: {cmd}", "§b SCMD §r")
        return self.cached_sendcmd(cmd, waitForResp, timeout)

    def special_monitor_sendwscmd(self, cmd: str, waitForResp: bool = False, timeout: float = 30):
        if self.instant_monitor:
            Print.print_with_info(f"发送 §bWS§r 指令{'<有返回>' if waitForResp else ''}: {cmd}", "§b SCMD §r")
