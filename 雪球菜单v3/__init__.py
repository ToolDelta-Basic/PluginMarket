...
class AcquiredLock:
    def __init__(self, sys: "SnowMenuV3"):
        self.sys = sys
...
class Page:
    ...
    Args:
        ...
        ok_cb: 玩家抬头确认选项后的回调 (玩家: Player, 当前所在页数: int) -> 需要的返回值如下:
            Page 对象: 跳转到该静态页
            元组 (MultiPage对象, <页码数: int>): 跳转到该复合页的该页码
            True: 留在该页不退出菜单
            None: 直接退出菜单
        exit_cb: 玩家低头退出菜单后的回调 (玩家: Player) -> None
        leave_cb: 玩家退出游戏后的回调 (玩家: Player) -> None
    ...
    next_page_id: str
    page_texts: str
    ok_cb: Callable[[Player], bool]
    exit_cb: Callable[[Player], None] = lambda _: None
    leave_cb: Callable[[Player], None] = lambda _: None
...
class MultiPage:
    ...
    Args:
        page_id: 菜单页面的ID, 不要与其他页面的id重复
        page_cb: 显示动态菜单页的方法: (玩家: Player, 当前页码数: int) -> 需要的返回值如下:
           str: 菜单页文本内容
    ...
    page_id: str
    page_cb: Callable[[Player, int], str | None]
    ok_cb: Callable[
        [Player, int], None | Literal[True] | "Page" | tuple["MultiPage", int]
    ]
    exit_cb: Callable[[Player], bool | None] = lambda _: None
    leave_cb: Callable[[Player], bool | None] = lambda _: None
    parent_page_id: str | None = None


PAGE_OBJ = Page | MultiPage
"菜单类"
PAGE_DISPLAY_OBJ = str | Callable[[Player, int], str | None]
"显示菜单页"
menu_patterns = [0, "undefined", "undefined", "undefined", "undefined"]
"菜单最大页码数, 菜单头, 菜单选项-未被选中, 菜单选项-被选中, 菜单尾"
main_page_menupages: dict[int, list[
    tuple[PAGE_OBJ | Callable[[Player], bool], str | Callable[[Player], str]]
]] = {}
"具有优先级的主菜单: 菜单类/菜单cb, 显示字(字符串 or 显示回调)"

def unfold_priority_dict(d: dict[int, list[T]]) -> list[T]:
    ls: list[T] = []
...
def default_page_show(player: Player, page: int):
    main_page_menus = unfold_priority_dict(main_page_menupages)
    max_length = len(main_page_menus)
    if page > max_length - 1:
        return None
    fmt_kws = {"[当前页码]": page + 1, "[总页码]": max_length, "[玩家名]": player.name}
    ...
    for page_cb, disp in cur_pages:
        ...
        show_texts.append(
            utils.simple_fmt(
                {"[选项文本]": text},
                menu_patterns[
                    3 if page == main_page_menus.index((page_cb, disp)) else 2
                ],
            )
        )
    if len(show_texts) == 1:
...
def default_page_okcb(player: Player, page: int):
    page_cb = unfold_priority_dict(main_page_menupages)[page][0]
...
class SnowMenuV3(Plugin):
    ...
    "使用 self.GetPluginAPI('雪球菜单v3').Page 来获取到这个菜单类, 下同"
    Page = Page
    MultiPage = MultiPage
    ...
    def register_main_page(
        self,
        page_cb: PAGE_OBJ | Callable[[Player], bool],
        usage_text: str | Callable[[Player], str],
        priority: int = 0,
    ):
        """
        注册一个雪球菜单首页跳转链接
        ...
        main_page_menupages.setdefault(priority, []).append((page_cb, usage_text))

    def simple_select(
        self,
        player: Player,
        disp_func: Callable[[Player, int], str | None],
        default_page: int = 0,
    ) -> int | None:
        """
        ...
        if not player.online:
            return None
        self.game_ctrl.sendwocmd(f"/execute as {player.safe_name} at @s run tp ~~~~ 0")
        ...
        cb = _cb()
        page = MultiPage(
            page_id=f"simple-select({player.name},{disp_func}..)",
            page_cb=disp_func,
            ok_cb=cb.ok,
            exit_cb=cb.exit,
            leave_cb=cb.exit,
        )
        old_page = self.in_snowball_menu.get(player.name)
    ...
    def simple_select_dict(self, player: Player, mapping: dict[int, str]):
        """
        简单地使用雪球菜单选择选项, 返回所选择的选项(页数), 首页是第 0 页.
        Args:
            player: 玩家名
        ...
        page = MultiPage(
            page_id=f"simple-select({player.name},{next(iter(mapping.keys()))}..)",
            page_cb=page_cb,
            ok_cb=cb.ok,
            exit_cb=cb.exit,
            leave_cb=cb.exit,
        ...
        old_page = self.in_snowball_menu.get(player.name)
        self._dbg(f"{player.name} entered dict sel")
        self.set_player_page(player, page)
        if old_page is None:
            self.show_page_thread(player)
        result = cb.start()
        self._dbg(f"{player.name} sel page {result}")
        self.set_player_page(player, old_page)
        return result
    ...
    def on_def(self):
        ...
        chatbar.add_trigger(
            ...
            None,
            "初始化雪球菜单所需命令方块",
            lambda p, _: self.place_cbs(p) and None,
            op_only=True,
        )
        cb2bot.regist_message_cb(
            "snowball.menu.use", lambda args: self.on_next_page(args[0])
        )
        cb2bot.regist_message_cb(
    ...
    def on_player_join(self, player: Player):
        self.game_ctrl.sendwocmd(f"/tag {player.safe_name} remove snowmenu")

    def on_inject(self):
        self.game_ctrl.sendwocmd("/tag @a remove snowmenu")

    def set_player_page(
        self, player: Player, page: Page | MultiPage | None, page_sub_id: int = 0
    ):
        if page is None:
            self.game_ctrl.sendwocmd(f"/tag {player.safe_name} remove snowmenu")
            if player.name in self.in_snowball_menu.keys():
                del self.in_snowball_menu[player.name]
            if player.name in self.multi_snowball_page.keys():
                del self.multi_snowball_page[player.name]
        else:
            if player.name not in self.in_snowball_menu.keys():
                self.game_ctrl.sendwocmd(f"/tag {player.safe_name} add snowmenu")
            self.in_snowball_menu[player.name] = page
            if isinstance(page, MultiPage):
                self.multi_snowball_page[player.name] = page_sub_id

    def on_player_leave(self, player: Player):
        if player.name in self.in_snowball_menu.keys():
            cb = self.in_snowball_menu[player.name].leave_cb
            if cb is not None:
                cb(player)
    ...
    def _getPlayer(self, playername: str):
        player = self.frame.get_players().getPlayerByName(playername)
        assert player, f"{playername} not found"
        return player

    @utils.thread_func("放置雪球菜单命令块")
    def place_cbs(self, playername: str):
        player = self._getPlayer(playername)
    ...
    def on_next_page(self, playername: str):
        main_page_menus = unfold_priority_dict(main_page_menupages)
        player = self._getPlayer(playername)
        self.game_ctrl.sendwocmd(f"/tag {player.safe_name} add snowmenu")
        self.game_ctrl.sendwocmd(
            f'/replaceitem entity @a[name="{player.name}",m=!1] slot.weapon.mainhand 0 keep snowball 1 0 {{"item_lock":{{"mode":"lock_in_inventory"}}}}'
        )
        now_page = self.in_snowball_menu.get(player.name)
        if now_page is None:
            self.game_ctrl.sendwocmd(
                f"/execute as {player.safe_name} at @s run tp ~~~~ 0"
            )
            self.in_snowball_menu[player.name] = self.reg_pages["default"]
            self.multi_snowball_page[player.name] = 0
            self.show_page_thread(player)
        elif isinstance(now_page, Page):
            next_page = now_page.next_page_id
            self.in_snowball_menu[player.name] = self.reg_pages[next_page]
        elif isinstance(now_page, MultiPage):
            if now_page.page_id == "default":
                self.multi_snowball_page[player.name] += 1
                if self.multi_snowball_page[player.name] >= len(main_page_menus):
                    self.multi_snowball_page[player.name] = 0
            else:
                self.multi_snowball_page[player.name] += 1
                r = now_page.page_cb(player, self.multi_snowball_page[player.name])
                if r is None:
                    self.multi_snowball_page[player.name] = 0
                    r = now_page.page_cb(player, 0) or "<ERROR PAGE>"
                    player.setActionbar(r)
        self.show_page(player)

    @utils.thread_func("展示雪球菜单页")
    def show_page_thread(self, player: Player):
        if player.name not in self.in_snowball_menu.keys():
            fmts.print_war(f"{player.name} 的菜单显示不正常: 可能是立刻退出了！")
        while player.name in self.in_snowball_menu.keys():
            self.show_page(player)
            time.sleep(1)

    def show_page(self, player: Player):
        mp = self.in_snowball_menu[player.name]
        if isinstance(mp, Page):
            page_text = mp.page_texts
        else:
            try:
                page_text = mp.page_cb(player, self.multi_snowball_page[player.name])
            except Exception:
        ...
        if page_text is None:
            return
        player.setActionbar(page_text)

    @utils.thread_func("雪球菜单执行")
    def menu_confirm(self, playername: str):
        player = self._getPlayer(playername)
        self._dbg(f"{player.name} confirmed")
        # 确认选项
        if player.name not in self.in_snowball_menu.keys():
            fmts.print_war(f"玩家: {player.name} 雪球菜单确认异常: 不在雪球菜单页内")
            return
        page = self.in_snowball_menu[player.name]
        if isinstance(page, Page):
            res = page.ok_cb(player)
        else:
            res = page.ok_cb(player, self.multi_snowball_page[player.name])
        if res is None:
            self.remove_player_in_menu(player)
        else:
            self.game_ctrl.sendwocmd(
    ...
    def menu_escape(self, playername: str):
        player = self._getPlayer(playername)
        if player.name not in self.in_snowball_menu.keys():
            fmts.print_war(f"玩家: {player.name} 雪球菜单退出异常: 不在雪球菜单页内")
            self.game_ctrl.sendwocmd(f"/tag {player.safe_name} remove snowmenu")
            return
        cb = self.in_snowball_menu[player.name].exit_cb
        _parent = self.in_snowball_menu[player.name].parent_page_id
        if _parent is None:
            player.setActionbar(self.cfg["菜单退出提示"])
            self.remove_player_in_menu(player)
        else:
            self.in_snowball_menu[player.name] = self.reg_pages[_parent]
            self.multi_snowball_page[player.name] = 0
        self.game_ctrl.sendwocmd(f"execute as {player.safe_name} at @s run tp ~~~ ~ 0")
        if cb is not None:
            cb(player)

    def remove_player_in_menu(self, player: Player):
        self._dbg(f"{player.name} exit menu")
        del self.in_snowball_menu[player.name]
        if player.name in self.multi_snowball_page.keys():
            del self.multi_snowball_page[player.name]
        self.game_ctrl.sendwocmd(f"/tag {player.safe_name} remove snowmenu")
...
entry = plugin_entry(SnowMenuV3, "雪球菜单v3")
