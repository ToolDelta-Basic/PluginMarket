...
async def handle_control(ws: WS, UUID, HOST):
    while True:
        ...
        if msg.startswith("CHANNEL:"):
            ...
            fmts.print_inf(f"New channel {channel_id} started.")
            ...
            try:
                ...
                fmts.print_err(f"Channel {channel_id} connections failed: {exc}")
...
async def main(UUID, HOST):
    while True:
        try:
            ...
            fmts.print_err(f"Error: {exc}")
            await asyncio.sleep(5)


@utils.thread_func("WS 转发器")
def launch_ws_client(UUID, HOST):
    try:
