...
class CloudBlacklist(Plugin):
    ...
    def _setup_commands(self):
        triggers = [
            ...
            (
                "更新黑名单服务器云黑数据",
                self.Update_blacklist_server,
                "将本地的黑名单数据上传到自己的黑名单服务器",
            ),
            (
                "添加黑名单",
                self.add_black_list,
                "将曾经加入过服务器的玩家添加进本地黑名单",
            ),
    ...
    def _safe_request(self, url: str) -> requests.Response:
        try:
            response = requests.get(url, timeout=self._cfg["超时时间（秒）"])
    ...
    def Create_blacklist_server(self, args: List[str]):
        try:
            ...
            target_dir = os.path.abspath(
                os.path.join(current_dir, "..", "..", "..", "ToolDelta基本配置.json")
            )
            ...
            try:
                response = requests.post(
                    upload_url,
                    json={"password": password, "blacklist": blacklist_data},
