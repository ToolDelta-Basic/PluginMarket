if __name__ == "plugins_required":
    ...
    from tooldelta import (
        plugin_entry, Plugin, ToolDelta, Player,
        Chat, FrameExit, fmts, InternalBroadcast
    )
    from tooldelta.constants import PacketIDS
    from tooldelta.plugin_load.exceptions import (
        NotValidPluginError, PluginAPINotFoundError,
        PluginAPIVersionError, PluginSkip
    )
    from tooldelta.mc_bytes_packet.base_bytes_packet import BaseBytesPacket
    from tooldelta.plugin_load.classic_plugin.event_cbs import (
        on_chat_cbs, ON_ERROR_CB, on_active_cbs, on_frame_exit_cbs,
        on_player_join_cbs, on_player_leave_cbs, on_preload_cbs,
        on_reloaded_cbs, dict_packet_funcs, bytes_packet_funcs, broadcast_listener
    )
    import time
    import importlib
    ...
    class HazelmeowPluginPerformanceAnalyzer(Plugin):
        ...
        def on_preload(self) -> None:
            ...
            self.frame.add_console_cmd_trigger(
                ["pf switch"], "<packet/broadcast>", "切换 是否显示 数据包/广播 插件",
                self.on_console_switch
            )
            setattr(self.chatbar, "_self_perf", self)  # noqa: B010
            for index, (plugin, func) in enumerate(on_chat_cbs):
                if func == self.chatbar.on_player_message:
                    on_chat_cbs[index] = \
                        plugin, MethodType(self.on_player_message, self.chatbar)
        ...
        def on_console_switch(self, args: list[str]) -> None:
            if (len(args) == 0) or (args[0] == ""):
                fmts.print_err('语法错误: 意外的 "": 出现在 "perf switch >><<"')
                return
            if len(args) > 1:
                fmts.print_err(
                    '语法错误: 意外的 "%s": 出现在 "perf switch %s >>%s<< %s"'\
                        % (args[1], args[0], args[1], " ".join(args[2:]))
                )
                return
            if args[0] not in ["packet", "broadcast"]:
                fmts.print_err(
                    '语法错误: 意外的 "%s": 出现在 "perf switch >>%s<<"'\
                        % (args[0], args[0])
                )
                return
            ...
            if action == "packet":
                ...
                fmts.print_suc(
                    f"{self.name}: 将"
                    '会' if self.analyze_packet else '不'
                    "对数据包插件进行分析."
                )
            if action == "broadcast":
                ...
                fmts.print_suc(
                    f"{self.name}: 将"
                    '会' if self.analyze_broadcast else '不'
                    "对广播插件进行分析."
                )
        ...
        def auto_stop_analyze(self) -> None:
            time.sleep(self.max_record_sec +0.1)
            if self.analyzing and (
                (time.time() -self.analyze_start_time) >= self.max_record_sec
            ):
                self.end_analyze()
        ...
        def _any(self, x: Any) -> Any:
            return x

        def _execute_preload(self, onerr: ON_ERROR_CB) -> None:
            self._execute("preload", None, onerr)

        def _execute_active(self, onerr: ON_ERROR_CB) -> None:
            self._execute("active", None, onerr)

        def _execute_player_join(self, player: Player, onerr: ON_ERROR_CB) -> None:
            self._execute("player_join", player, onerr)

        def _execute_chat(self, chat: Chat, onerr: ON_ERROR_CB) -> None:
            self._execute("chat", chat, onerr)

        def _execute_player_leave(self, player: Player, onerr: ON_ERROR_CB) -> None:
            self._execute("player_leave", player, onerr)

        def _execute_frame_exit(self, evt: FrameExit, onerr: ON_ERROR_CB) -> None:
            self._execute("frame_exit", evt, onerr)

        def _execute_reloaded(self, onerr: ON_ERROR_CB) -> None:
        ...
        def _create_base_data(self, message: str) -> dict:
            plugin_data = {}
            base_data = {
                "message": message,
                "timestamp": time.time(),
        ...
        @staticmethod
        def _before_func_call(
            plugin_data: dict, plugin_name: str, thread: bool
        ) -> None:
        ...
        @staticmethod
        def _after_func_call(
            plugin_data: dict, plugin_name: str, exc: Optional[BaseException] = None
        ) -> None:
        ...
        @staticmethod
        def _check_plugin_env(plugin: Plugin, exc: Optional[BaseException]) -> None:
            ...
            if isinstance(exc, PluginAPIVersionError):
                fmts.print_err(
                    f"插件 {plugin.name} 需要前置组件 {exc.name} v{exc.m_ver}, "
                    f"但是现有版本过低 v{exc.n_ver}"
                )
                raise SystemExit from exc

        @staticmethod
        def _print_plugin_data(
            message: str, plugin_data: dict,
            blocked: bool, in_thread_func: bool
        ) -> None:
            if not in_thread_func:
                ...
                if message == "[IMPORT]" and plugin_name_last.startswith("."):
                    return
                ...
                if count == 1:
                    string = f"[ SUM  ]{message} ({plugin_name_last}) " \
                             F"({time_spend_ms_total:.2f}ms)"
                    string = "§c" + string if exception else "§b" + string
                else:
                    string = f"§1§l[ SUM  ]{message} ({time_spend_ms_total:.2f}ms) " \
                              "(without @utils.thread_func)§r" +string
                ...
                plugin_name, data = next(iter(plugin_data.items()))
                thread_name = data["thread"]

                if (submodule := data.get("submodule")):
                    if len(submodule) > 1:
                        raise NotImplementedError
                    plugin_name, data = next(iter(submodule.items()))

                time_spend_ms = (data["end"] -data["start"]) *1000
                exception = data["exc"]

                string = f"§2§l[THREAD]{message} ({plugin_name}) " \
                         f"({time_spend_ms:.2f}ms) (in thread {thread_name})"
                if exception:
        ...
        def _execute(self,
            ...
            if execute_name not in [
                ...
                raise ValueError(f"{self.name}: 执行名称不正确.")
            if not self.analyzing:
                if param is None:
        ...
        def _execute_packet(self,
            execute_name: str,
            pktID: int,
            pkt: Union[dict, BaseBytesPacket],
            onerr: ON_ERROR_CB,
        ) -> bool:
            if execute_name not in [
                "dict", "bytes"
            ...
            if not analyzing:
                return bool(
                    self.execute_name_2_orig_func[execute_name](pktID, pkt, onerr)
                )
            ...
            return self.analyzer(message, func_list, pkt, onerr, block = True)[-1]
        ...
        def _broadcast_event(self,
            ...
            for index, func in enumerate(func_list):
                ...
                if not isinstance(plugin, Plugin):
                    plugin.name = plugin.__class__.__name__
                func_list[index] = (plugin, func)
            return self.analyzer(message, func_list, evt, None, block = True)


        def import_module(self, name: str, package: Any = None) -> Any:
            if not self.analyzing:
            ...
            plugin.name = name
            module = self.analyzer(message, [(
                    plugin, lambda *args: self._orig_import_module(name, package)
                )], (name, package), None
            )
            return module[0]
        ...
        def analyzer(
            self, message: str,
            func_list: list[tuple[Plugin, Callable]],
            param: Any, onerr: Optional[Callable], block: bool = False
        ) -> list:
            ...
            try:
                for plugin, func in func_list:
                    ...
                    if not func_is_utils_thread:
                        res = self._analyzer(
                            message, func, plugin, plugin_data, param, thread = False
                        )
                        return_list.append(res)
                    else:
                        utils.thread_func(func._usage) \
                        (self._analyzer) \
                        (message, func._orig, plugin, plugin_data, param, func._usage)
                ...
                self._print_plugin_data(
                    message, plugin_data, block and bool(res), in_thread_func = False
                )
        ...
        def _analyzer(
            self, message: str, func: Callable,
            plugin: Plugin, plugin_data: dict, param: Any, thread: bool
        ) -> Any:
            args = []
            ...
            if thread and (param is not None) and isinstance(param, Chat):
                param._plugin_data = plugin_data[plugin_name]  # type: ignore  # noqa: PGH003
            ...
            try:
                ...
                if thread:
                    self._print_plugin_data(
                        message, {plugin_name: plugin_data[plugin_name]},
                        blocked = False, in_thread_func = thread
                    )
            return res
        ...
        def on_player_message(self: Any, chat: Chat) -> None:
            self = getattr(self, "_self_perf")  # noqa: B009
            if not self.analyzing:
                self.orig_on_player_message(chat)
                return
            msg = chat.msg
            player = chat.player
            plugin_data = chat._plugin_data # type: ignore  # noqa: PGH003
            submodule_data = {}
            ...
            for trigger in self.chatbar.chatbar_triggers:
                for trigger_str in trigger.triggers:
                    if msg.startswith(trigger_str):
                        ...
                        try:
                            submodule_data[plugin.name] = {
                                "start": time.time(), "end": None, "exc": None
                            }
                            with utils.ChatbarLock(player.name):
                                trigger_class_name = trigger.__class__.__name__
                                if "StandardChatbarTriggers" in trigger_class_name:
