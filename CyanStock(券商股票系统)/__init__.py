...
class StockBrokerPlugin(Plugin):
    ...
    CONFIG_DEFAULT = {
        ...
        "市场事件": [
            {"事件类型": "利好", "影响范围": "全局", "事件描述": "央行降息，市场流动性增加", "持续时间分钟": 8, "价格修正": 1.10},
            {"事件类型": "利好", "影响范围": "全局", "事件描述": "政策利好：资本市场改革方案", "持续时间分钟": 8, "价格修正": 1.10},
            {"事件类型": "利空", "影响范围": "全局", "事件描述": "外部市场大跌，A股承压", "持续时间分钟": 8, "价格修正": 0.90},
            {"事件类型": "利空", "影响范围": "全局", "事件描述": "监管层加强风险管控", "持续时间分钟": 8, "价格修正": 0.90},
            {"事件类型": "利好", "影响范围": "单券商", "事件描述": "财报超预期，业绩亮眼", "持续时间分钟": 8, "价格修正": 1.15},
            {"事件类型": "利空", "影响范围": "单券商", "事件描述": "遭遇监管调查", "持续时间分钟": 8, "价格修正": 0.85},
            {"事件类型": "利好", "影响范围": "单券商", "事件描述": "获得重大订单", "持续时间分钟": 8, "价格修正": 1.15},
            {"事件类型": "利空", "影响范围": "单券商", "事件描述": "高管离职引发担忧", "持续时间分钟": 8, "价格修正": 0.85},
        ],
    }
    ...
    def load_data(self):
        if self.data_file.exists():
            try:
                ...
                self.market_sentiment = data.get("market_sentiment", self.sentiment_initial)
    ...
    def reset_data(self):
        self.players = {}
        self.broker_stocks = {broker: self.initial_stock for broker in self.broker_names}
    ...
    def _flush_save(self):
        ...
        with self._save_lock:
            ...
            data = {
                ...
                "broker_stocks": self.broker_stocks,
    ...
    def add_kp(self, player_name: str, amount: int):
        if amount <= 0:
            return
        self.game_ctrl.sendcmd(f"/scoreboard players add {player_name} {self.scoreboard_name} {amount}")

    def remove_kp(self, player_name: str, amount: int):
        if amount <= 0:
            return
        self.game_ctrl.sendcmd(f"/scoreboard players remove {player_name} {self.scoreboard_name} {amount}")
    ...
    def on_active(self):
        try:
            self.game_ctrl.sendwocmd(f"/scoreboard objectives add {self.scoreboard_name} dummy")
        except Exception:
            pass
        ...
        if self.active_events:
            ...
            if active:
                ...
                for ev in active:
                    ...
                    broadcast += f"  {t_color}[{ev['type']}] §f{ev['desc']} §7({scope}, §e{remaining}分钟§7)\n"
    ...
    def _trigger_market_event(self):
        ...
        with self._data_lock:
            ...
            delta = self.sentiment_delta_global if event["scope"] == "all" else self.sentiment_delta_single
            if event["type"] == "利好":
        ...
        if self.active_events:
            ...
            for ev in events_snapshot:
                ...
                lines.append(
                    ...
                    + f"  §7范围：§f{scope}  §7剩余：§e{remaining}§f分钟  §7修正：§e{ev['modifier']:.0%}"
                )
        else:
    ...
    def _add_trade_volume(self, uuid: str, amount: int, player_name: str = ""):
        ...
        if old_tier != new_tier and new_tier["min_volume"] > 0 and player_name:
            self.game_ctrl.say_to(
                ...
                + f"§f买入折扣：§a{new_tier['buy_discount']:.0%}  §f卖出加成：§a{new_tier['sell_bonus']:.0%}"
            )
    ...
    def show_vip_status(self, player: Player, show_nav: bool = True) -> str:
        ...
        lines = [
            ...
            f"§f当前：{tier['color']}§l{tier['name']}§r §7(累计 §e{volume}§7 {self.scoreboard_name})",
            "",
        ]
        # 直接列出每个等级的加成
        for t in self.vip_tiers:
            ...
            if t["buy_discount"] > 0 or t["sell_bonus"] > 0:
                lines.append(
                    f"{marker}§f{t['color']}{t['name']}§r §7入§a-{t['buy_discount']:.0%} §7出§a+{t['sell_bonus']:.0%}"
                )
            else:
        ...
        if next_tier:
            ...
            lines.append(f"§7再交易 §e{need}§7 {self.scoreboard_name} 升至 {next_tier['color']}{next_tier['name']}§r")
        else:
            lines.append("§7已满级")
        if show_nav:
            lines.append(self._nav_tip())
    ...
    def _event_loop(self):
        ...
        while self._event_running:
            ...
            if random.random() < self._event_chance:
                ...
                self.game_ctrl.say_to("@a", self.H2.format("§6市场播报") + "\n§f当前市场平稳，无重大事件")
    ...
    def show_kp_balance(self, player: Player, show_nav: bool = True) -> str:
        ...
        text = (
            ...
            + self.TIP.format(f"买入股票时自动扣除 {self.scoreboard_name}，卖出股票时自动获得 {self.scoreboard_name}")
        )
        if show_nav:
            text += "\n" + self._nav_tip()
        return text
    ...
    def buy_stock(self, player: Player, broker: str, quantity: int) -> str:
        ...
        with self._data_lock:
            if self.broker_stocks.get(broker, 0) < quantity:
                return f"§c{broker} §f库存不足\n§f当前仅剩：§e{self.broker_stocks.get(broker, 0)} §f股"
            tier = self._get_vip_tier(uuid)
            base_cost = self.brokers[broker]["cost"] * quantity
            cost = int(base_cost * (1 - tier["buy_discount"]))
            kp = self.get_kp(player.name)
            if kp < cost:
                return f"§c{self.scoreboard_name} 不足\n§f需要：§e{cost} {self.scoreboard_name}  §f当前：§e{kp} {self.scoreboard_name}"
            current_hold = self.get_holdings(uuid, broker)
            if current_hold + quantity > self.max_holdings:
                return f"§c该券商持仓将超过上限 §e{self.max_holdings} §f股\n§f当前：§e{current_hold} §f股"
        ...
        discount_text = f" §7(等级{tier['name']} §a-{tier['buy_discount']:.0%}§7)" if tier["buy_discount"] > 0 else ""
        ...
        for broker in self.broker_names:
            ...
            if eff_min != base_min or eff_max != base_max:
                if eff_max > base_max:
                    ...
                    price_tag = " §c↓"
            lines.append(
                f"§e• §f{broker} §7存§f{stock} §7价§e{info['cost']} §7险{risk_color} §7出§a{eff_min}-{eff_max}{price_tag}"
            )
        lines.append("")
    ...
    def get_broker_detail(self, uuid: str, broker: str) -> str:
        ...
        lines = [
            ...
            f"§f等级折扣：§a-{tier['buy_discount']:.0%} §7(实付 §e{int(info['cost'] * (1 - tier['buy_discount']))} {self.scoreboard_name}§7)",
            f"§f风险等级：{self._effective_risk_color(broker)}",
            f"§f基准收益：§7{info['sell_min']} ~ {info['sell_max']} {self.scoreboard_name} §f/股",
        ...
        if broker_events:
            ...
            for ev in broker_events:
                remaining = int((ev["expiry"] - now) / 60)
                t_color = "§a" if ev["type"] == "利好" else "§c"
                lines.append(f"  {t_color}[{ev['type']}] §f{ev['desc']} §7(§e{remaining}分钟§7, §e{ev['modifier']:.0%}§7)")
    ...
    def show_broker_list(self, player: Player, state: int, menu_page: int = 0):
        ...
        for idx, name in enumerate(self.broker_names, 1):
            ...
            if eff_max > info["sell_max"]:
                ...
                price_tag = " §c↓"
            lines.append(
                f"§e{idx}. §f{name} §7价§e{info['cost']} §7险{risk_color} §7卖{eff_min}-{eff_max}{price_tag}"
            )
        lines.append("")
    ...
    def show_main_menu(self, player: Player, page: int = 0):
        ...
        lines = [
            ...
            f"§f{self.scoreboard_name}:§e{kp}  §f等级:{tier['color']}{tier['name']}§r  §f情绪:{color}{self.market_sentiment}§r",
            "",
            *item_lines,
        ]
        if nav:
            lines.extend(["", self.TIP.format(f"第{page+1}/{total_pages}页  {nav}  输入0/q退出")])
        else:
    ...
    def handle_main_menu(self, player: Player, choice):
        ...
        if choice == 1:
            ...
            if not has_stock:
                ...
                self.cancel_session(uuid)
                return
            self.show_broker_list(player, self.STATE_SELLALL_SELECT_BROKER, menu_page=page)
        elif choice == 6:
            ...
            if err:
                ...
                self.cancel_session(uuid)
                return
            self.show_broker_list(player, self.STATE_DETAIL_SELECT_BROKER, menu_page=page)
        elif choice == 9:
            ...
            self.game_ctrl.say_to(player.name, f"§c无效选择，请输入 §e1-{len(self.MENU_ITEMS)} §c选择功能")
    ...
    def handle_broker_selection(self, player: Player, choice: int):
        ...
        if state == self.STATE_BUY_SELECT_BROKER:
            ...
            self.game_ctrl.say_to(player.name, self.H2.format("请输入买入数量") + "\n" + self._nav_tip())
            session["state"] = self.STATE_BUY_INPUT_QUANTITY

        elif state == self.STATE_SELL_SELECT_BROKER:
            holdings = self.get_holdings(uuid, broker)
            if holdings <= 0:
                self.game_ctrl.say_to(player.name, self.RED.format(f"你在 {broker} 没有股票可卖出"))
                self.cancel_session(uuid)
                return
            self.game_ctrl.say_to(
                player.name,
    ...
    def handle_quantity_input(self, player: Player, quantity: int):
        ...
        if state == self.STATE_BUY_INPUT_QUANTITY:
            ...
            with self._data_lock:
                sell_min, sell_max = self._get_effective_sell_range(broker)
                base_total = sum(random.randint(sell_min, sell_max) for _ in range(quantity))
                tier = self._get_vip_tier(uuid)
                total = int(base_total * (1 + tier["sell_bonus"]))
            ...
            self.save_data()
            bonus_text = f" §7(等级{tier['name']} §a+{tier['sell_bonus']:.0%}§7)" if tier["sell_bonus"] > 0 else ""
            self.game_ctrl.say_to(
                player.name,
    ...
    # ---------- 持仓 ----------
    def show_holdings(self, uuid: str, player_name: str = "", show_nav: bool = True) -> str:
        err = self.require_registered(uuid)
        if err:
            return err
        ...
        lines = [
            ...
            f"§f当前 {self.scoreboard_name}：§e{kp}",
            f"§f当前等级：{tier['color']}§l{tier['name']}§r §7(累计交易 §e{volume}§7 {self.scoreboard_name})",
            "",
            "§f持有股票："
        ]
        has_stock = False
        for broker in self.broker_names:
            ...
            if shares > 0:
                eff_min, eff_max = self._get_effective_sell_range(broker)
                lines.append(f"§e• §f{broker}  §a{shares} §f股  §7收益 §a{eff_min}~{eff_max} {self.scoreboard_name}")
                has_stock = True
        if not has_stock:
            lines.append("§7暂无股票")
        lines.append("")
    ...
    def on_chat(self, chat: Chat):
        ...
        if session:
            ...
            if state == self.STATE_MAIN_MENU:
                if msg in ("+", "＋"):
                    ...
                    total = (len(self.MENU_ITEMS) + self.PAGE_SIZE - 1) // self.PAGE_SIZE
                    if cur < total - 1:
            ...
            try:
                ...
                elif state in (self.STATE_BUY_SELECT_BROKER, self.STATE_SELL_SELECT_BROKER,
                               self.STATE_SELLALL_SELECT_BROKER, self.STATE_DETAIL_SELECT_BROKER):
                    self.handle_broker_selection(chat.player, num)
                elif state in (self.STATE_BUY_INPUT_QUANTITY, self.STATE_SELL_INPUT_QUANTITY):
        ...
        if msg == ".走势":
            ...
            part1 = "\n".join(lines[:8])
            part2 = "\n".join(lines[8:])
            self.say_lines(pname, part1)
            self.say_lines(pname, part2)
            return
