...
class QQLinkerConfigMixin:
    ...
    MENU_EXIT_TRIGGERS_DEFAULT: ClassVar[list[str]] = [".", "。", "q"]
    MENU_BACK_TRIGGERS_DEFAULT: ClassVar[list[str]] = ["!", "！"]
    ...
    def migrate_config(self, raw_cfg: Any):  # skipcq: PY-R1000
        ...
        if isinstance(cloud_cfg, dict):
            new_cfg["云链设置"]["是否启用该通道"] = self._normalize_bool(
                cloud_cfg.get("是否启用该通道"), False
            )
            new_cfg["云链设置"]["地址"] = str(
        ...
        qqbot_cfg = raw_cfg.get("官机设置", {})
        if isinstance(qqbot_cfg, dict):
            new_cfg["官机设置"]["是否启用该通道"] = self._normalize_bool(
                qqbot_cfg.get("是否启用该通道"), False
            )
            for key in ("AppID", "AppSecret"):
                value = qqbot_cfg.get(key, "")
                new_cfg["官机设置"][key] = (
    ...
    def apply_ultra_runtime_config(self, raw_config: Any) -> str:
        ...
        old_qqbot_cfg = deepcopy(getattr(self, "cfg", {}).get("官机设置", {}))
        old_group_order = list(getattr(self, "group_order", []))
        ...
        groups_changed = old_group_order != self.group_order
        if old_qqbot_cfg != self.cfg.get("官机设置", {}) or groups_changed:
            self.reload_qqbot_connection()
        if (
            old_cloud_cfg != self.cfg.get("云链设置", {})
            or old_qqbot_cfg != self.cfg.get("官机设置", {})
            or groups_changed
        ):
            return "Ultra 配置已动态载入，消息通道已按新配置重载"
