...
云链自动连接采用滚动一小时失败窗口，每小时最多记录 5 次连接失败：

- 前 4 次失败分别按原有延迟自动重连，并显示当前失败次数。
- 第 5 次失败后暂停自动重连，不再持续输出彩色连接报错。
- Ultra 会等待到最早一次失败记录离开一小时窗口后，自动恢复连接尝试。
- 同一个 WebSocket 会话重复触发错误回调时只记录和输出一次。
- 云链成功连接后立即清空失败次数。
- 关闭云链通道、重载连接或退出插件时，会立即中断正在进行的限流等待。
...
该限制只控制云链 WebSocket 的自动重连频率，不影响官机通道和已经建立连接后的正常消息转发。

## 13. 官机通道身份限制

QQ 官方机器人群消息事件只能提供成员 OpenID，不能提供成员真实 QQ 号。Ultra 会使用内部会话身份完成回复、菜单等待和官方群角色鉴权，但不会把该内部身份当作真实 QQ 号写入绑定数据或管理员数据。

因此，官机通道禁用以下依赖真实 QQ 号的功能：

| 功能 | 官机通道行为 |
| --- | --- |
| QQ 与游戏账号绑定 | 发送绑定触发词或通过官机身份调用绑定流程时，提示改用云链通道 |
| QQ 群管理员管理菜单 | 直接发送管理员菜单触发词，或从 Help 管理功能中进入该菜单时，提示改用云链通道 |
| 普通群成员公会菜单 | 该菜单需要按 QQ 号查询绑定游戏账号，因此官机普通成员入口提示改用云链绑定通道 |
...
不依赖真实 QQ 号的功能仍可继续通过官机使用，包括查看玩家列表、执行 Minecraft 指令、配置中心、背包查询、Orion 管理、白名单与管理员检测、任务系统、领地系统，以及拥有官方群管理角色时使用公会管理菜单。云链通道的绑定和管理员菜单行为不受影响。

## 14. 官机连接报错码

官机通道的鉴权、网关连接和心跳发生异常时，会按云链通道相同的控制台卡片样式输出。每条错误至少包含 Ultra 错误码和错误说明；QQ 平台返回 HTTP 状态、平台错误码或 WebSocket 关闭码时，会一并保留原始信息。
...
| 错误码 | 含义 |
| --- | --- |
| `QQBOT-1001` | `websocket-client` 依赖不可用，官机网关无法启动 |
| `QQBOT-1002` | 官机通道已启用，但 `AppID` 或 `AppSecret` 不完整 |
| `QQBOT-1003` | QQ 官方机器人通道启动失败 |
| `QQBOT-1101` | 获取 AccessToken 的 HTTP 请求失败；附带 HTTP 状态码和响应内容 |
| `QQBOT-1102` | AccessToken 响应缺少 `access_token`；如平台返回错误码和错误信息则一并输出 |
| `QQBOT-1103` | 获取 AccessToken 时发生其它网络或运行时异常 |
| `QQBOT-1201` | 获取官机网关地址的 HTTP 请求失败；附带 HTTP 状态码和响应内容 |
| `QQBOT-1202` | 官机网关响应缺少 `url` |
| `QQBOT-1203` | 获取官机网关地址时发生其它异常 |
| `QQBOT-1301` | 官机网关连接循环异常 |
| `QQBOT-1302` | WebSocket 错误回调收到异常 |
| `QQBOT-1303` | WebSocket 异常关闭；附带关闭码和关闭原因 |
| `QQBOT-1304` | QQ 网关返回操作码 `9`，表示鉴权或会话参数无效 |
| `QQBOT-1401` | 官机网关心跳发送失败 |

这些 `QQBOT-*` 是群服互通云链版 Ultra 自身的稳定错误码，用于定位失败阶段；HTTP 状态码、QQ 平台错误码和 WebSocket 关闭码属于上游原始报错信息，两者应结合排查。
