...
class QQLinkerConfigEditorMixin:
    ...
    CONFIG_MENU_TRIGGERS: ClassVar[list[str]] = [
        "配置中心",
        "配置菜单",
        "群服配置",
    ]
    ...
    def _edit_config_file_whole(  # skipcq: PY-R1000
        ...
        item_path = item["path"]
        try:
            with open(item_path, encoding="utf-8-sig") as file:
                content = file.read()
    ...
    def _restore_config_backup(
        ...
        try:
            ...
            with open(current_item["path"], encoding="utf-8-sig") as file:
    ...
    def _apply_runtime_config_file(  # skipcq: PY-R1000
        ...
        try:
            if config_name == self.name:
                ...
                self._runtime_config_file_state = self.runtime_config_file_state(
                    item["path"])
                return message
            if (
                config_name == "白名单&管理员检测云链联动版"
                and self.whitelist_checker is not None
                and hasattr(self.whitelist_checker, "apply_runtime_config")
            ):
                self.whitelist_checker.apply_runtime_config(config_items)
            ...
            if (
                ...
                and hasattr(self.land_system, "apply_runtime_config")
            ):
                self.land_system.apply_runtime_config(config_items)
                return "领地系统配置已动态载入"
            if (
