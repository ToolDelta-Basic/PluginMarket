...
try:
    ...
    class ProxyTimeoutError(Exception):
        """Fallback proxy timeout error used when python-socks is unavailable."""  # noqa: E501

        __slots__ = ()

    class ProxyConnectionError(Exception):
        """Fallback proxy connection error used when python-socks is unavailable."""  # noqa: E501
...
class proxy_info:
    ...
    def __init__(self, **options):
        ...
        if self.proxy_host:
            ...
            # If timeout is not specified, python-socks defaults to 60 seconds.
            self.proxy_timeout = options.get("http_proxy_timeout")
...
def _start_proxied_socket(url: str, options, proxy) -> tuple:
    """Open a socket through a SOCKS proxy and wrap it for TLS if needed."""
...
def connect(url: str, options, proxy, prepared_socket):
    """Create and return a socket connected to the websocket endpoint."""
    # Use _start_proxied_socket() only for socks4 or socks5 proxy
    # Use _tunnel() for http proxy
    # NOTE: HTTP proxying still uses _tunnel() until python-socks is adopted
    # for that path as well.
    if proxy.proxy_host and not prepared_socket and proxy.proxy_protocol != "http":  # noqa: E501
        return _start_proxied_socket(url, options, proxy)

    hostname, port_from_url, resource, is_secure = parse_url(url)

    if prepared_socket:
        return prepared_socket, (hostname, port_from_url, resource)
...
def _get_addrinfo_list(hostname, port: int, is_secure: bool, proxy) -> tuple:
    """Resolve the address list used for a direct or proxied connection."""
    ...
    try:
        # On Windows 10, getaddrinfo without socktype can return socktype 0,
        # which later breaks socket creation. Force SOCK_STREAM explicitly.
        if not phost:
        ...
        # Apply the same SOCK_STREAM workaround for the proxy endpoint.
        addrinfo_list = socket.getaddrinfo(
...
def _open_socket(addrinfo_list, sockopt, timeout):
    """Try each resolved address until a socket connection succeeds."""
    err = None
...
def _wrap_sni_socket(
    ...
    if not context:
        ...
        #   ?highlight=sslkeylogfile#context-creation
        # * https://docs.python.org/3.8/library/ssl.html
        #   ?highlight=sslkeylogfile#ssl.SSLContext.keylog_filename
        context.keylog_filename = os.environ.get("SSLKEYLOGFILE")

        if sslopt.get("cert_reqs", ssl.CERT_NONE) != ssl.CERT_NONE:
            cafile = sslopt.get("ca_certs")
            capath = sslopt.get("ca_cert_path")
            if cafile or capath:
        ...
        # Python 3.10 switched PROTOCOL_TLS_CLIENT defaults to
        # cert_reqs=CERT_REQUIRED and check_hostname=True.
        # If both are disabled, set check_hostname before verify_mode.
        # See:
...
def _ssl_socket(sock: socket.socket, user_sslopt: dict, hostname):
    """Apply TLS settings and wrap the socket for secure websocket use."""
    ...
    if (
        ...
        and user_sslopt.get("ca_certs") is None
    ):
        sslopt["ca_certs"] = cert_path
    elif (
        cert_path
        and os.path.isdir(cert_path)
        and user_sslopt.get("ca_cert_path") is None
    ):
        sslopt["ca_cert_path"] = cert_path

    if sslopt.get("server_hostname"):
...
def _tunnel(sock: socket.socket, host, port: int, auth) -> socket.socket:
    """Open an HTTP CONNECT tunnel through the configured proxy."""
    debug("Connecting proxy...")
    connect_header = f"CONNECT {host}:{port} HTTP/1.1\r\n"
    connect_header += f"Host: {host}:{port}\r\n"

    # NOTE: Only basic proxy authentication is implemented here.
    if auth and auth[0]:
        auth_str = auth[0]
        if auth[1]:
            auth_str += f":{auth[1]}"
...
def read_headers(sock: socket.socket) -> tuple:
    """Read an HTTP status line plus headers from the socket."""
    status = None
    status_message = None
    headers: dict = {}
