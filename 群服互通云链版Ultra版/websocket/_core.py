...
class WebSocket:
    ...
    This class is based on the WebSocket protocol
    ...
    def __next__(self):
        """Return the next received websocket message."""
        return self.recv()

    def next(self):
        """Backward-compatible alias for ``__next__``."""
    ...
    def gettimeout(self) -> Union[float, int, None]:
        ...
        timeout: int or float
             ...
             This value could be either float/integer.
        """
    ...
    def getsubprotocol(self):
        """Return the negotiated websocket subprotocol, if one exists."""
    ...
    def getstatus(self):
        """Return the HTTP status from the opening handshake."""
        if self.handshake_response:
    ...
    def getheaders(self):
        """Return the headers received during the opening handshake."""
        if self.handshake_response:
            return self.handshake_response.headers
        return None

    def is_ssl(self):
        """Return whether the underlying socket is wrapped with TLS."""
        try:
    ...
    def recv_data_frame(self, control_frame: bool = False) -> tuple:
        ...
        while True:
            ...
            if frame.opcode in (ABNF.OPCODE_PING, ABNF.OPCODE_PONG):
    ...
    def recv_frame(self):
        """Receive and decode a single frame from the server."""
    ...
    def abort(self):
        """Wake threads blocked in low-level receive operations."""
        if self.connected:
            self.sock.shutdown(socket.SHUT_RDWR)
    ...
    def _send(self, data: Union[str, bytes]):
        """Send raw bytes through the underlying socket."""
        return send(self.sock, data)

    def _recv(self, bufsize):
        """Receive raw bytes through the underlying socket."""
        try:
