...
RECONNECT_SETTINGS = {"interval": 0}


def setReconnect(reconnectInterval: int) -> None:
    """Update the default reconnect interval used by ``WebSocketApp``."""
    RECONNECT_SETTINGS["interval"] = reconnectInterval
...
class DispatcherBase:
    ...
    @staticmethod
    def timeout(seconds: Union[float, int, None], callback: Callable) -> None:
        """Sleep for ``seconds`` and then invoke ``callback``."""
        time.sleep(seconds)
        callback()

    @staticmethod
    def reconnect(seconds: int, reconnector: Callable) -> None:
        """Wait for ``seconds`` and then invoke the reconnect callback."""
        _logging.info(
    ...
    def read(
        ...
        try:
            while self.app.keep_running:
                if sel.select(self.ping_timeout) and not read_callback():
                    break
                check_callback()
        finally:
            sel.close()
...
class SSLDispatcher(DispatcherBase):
    ...
    def read(
        ...
        try:
            while self.app.keep_running:
                if self.select(sock, sel) and not read_callback():
                    break
                check_callback()
        finally:
            sel.close()
    ...
    def select(self, sock, sel: selectors.DefaultSelector):
        """Return ready events from the SSL socket, if any are available."""
        sock = self.app.sock.sock
        if sock.pending():
            return [
                sock,
...
class WrappedDispatcher:
    ...
    def read(
        ...
        """Delegate reads to the custom dispatcher and apply timeout checks."""
        self.dispatcher.read(sock, read_callback)
        if self.ping_timeout:
            self.timeout(self.ping_timeout, check_callback)

    def timeout(self, seconds: float, callback: Callable) -> None:
        """Delegate timeout handling to the wrapped dispatcher."""
        self.dispatcher.timeout(seconds, callback)

    def reconnect(self, seconds: int, reconnector: Callable) -> None:
        """Delegate reconnect scheduling to the wrapped dispatcher."""
...
class WebSocketApp:
    ...
    def __init__(
        ...
        prepared_socket: Optional[socket.socket] = None,
    ) -> None:
        """
        ...
        on_data: function
            ...
            The 1st argument is this class object.
            The 2nd argument is utf-8 string which we get from the server.
            The 3rd argument is data type.
            ABNF.OPCODE_TEXT or ABNF.OPCODE_BINARY will be came.
        ...
        self.prepared_socket = prepared_socket
    ...
    def _start_ping_thread(self) -> None:
        """Start the background ping thread used by ``run_forever``."""
    ...
    def _stop_ping_thread(self) -> None:
        """Stop the background ping thread and reset ping timestamps."""
        if self.stop_ping:
            self.stop_ping.set()
    ...
    def run_forever(  # skipcq: PY-R1000
        ...
        http_proxy_auth: tuple
            HTTP proxy auth information.
            Tuple of username and password. Default is None.
        ...
        def teardown(close_frame: ABNF = None):
            ...
            """
            # teardown() is called in many code paths to ensure resources are
            # cleaned up and on_close is fired. The flag and lock ensure that
            # cleanup still runs only once.
        ...
        def setSock(reconnecting: bool = False) -> None:
            """Create the socket, perform the handshake, and start reading."""
        ...
        try:
            ...
            if not custom_dispatcher and reconnect:
                while self.keep_running:
                    _logging.debug(
                        "Calling dispatcher reconnect [%s frames in stack]",
                        len(inspect.stack()),
                    )
                    dispatcher.reconnect(reconnect, setSock)
        except (KeyboardInterrupt, Exception) as e:
            _logging.info("tearing down on exception %s", e)
            teardown()
        finally:
    ...
    def create_dispatcher(
        ...
        """Create the dispatcher object used by ``run_forever``."""
    ...
    def _get_close_args(self, close_frame: ABNF) -> list:
        """Extract the close code and reason from a close frame if present."""
    ...
    def _callback(self, callback, *args) -> None:
        """Invoke a callback and forward callback failures to ``on_error``."""
        if callback:
            try:
                callback(self, *args)
