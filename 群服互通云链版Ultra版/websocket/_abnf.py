...
try:
    ...
    def _mask(mask_value: array.array, data_value: array.array) -> bytes:
        """Apply the accelerated XOR mask implementation."""
    ...
    def _mask(mask_value: array.array, data_value: array.array) -> bytes:
        """Apply the pure-Python XOR mask implementation."""
...
class ABNF:
    ...
    def __init__(
        ...
        """Construct an ABNF frame. Please check the RFC for arguments."""
        self.fin = fin
        self.rsv1 = rsv1
        self.rsv2 = rsv2
        self.rsv3 = rsv3
    ...
    def _is_valid_close_status(code: int) -> bool:
        """Return whether the close status code is allowed by the protocol."""
        return code in VALID_CLOSE_STATUS or (3000 <= code < 5000)

    def __str__(self) -> str:
        """Return a concise human-readable representation of the frame."""
    ...
    def _get_masked(self, mask_key: Union[str, bytes]) -> bytes:
        """Return the payload prefixed with its masking key."""
...
class frame_buffer:
    ...
    def __init__(
        ...
        """Initialize a frame buffer backed by the provided receive function."""  # noqa: E501
        self.recv = recv_fn
    ...
    def clear(self) -> None:
        """Reset the partially decoded frame state."""
        self.header: Optional[tuple] = None
        self.length: Optional[int] = None
        self.mask_value: Union[bytes, str, None] = None

    def has_received_header(self) -> bool:
        """Return whether the current frame header still needs to be read."""
        return self.header is None

    def recv_header(self) -> None:
        """Read and parse the fixed two-byte WebSocket frame header."""
    ...
    def has_received_length(self) -> bool:
        """Return whether the payload length still needs to be read."""
    ...
    def has_received_mask(self) -> bool:
        """Return whether the mask key still needs to be read."""
        return self.mask_value is None

    def recv_mask(self) -> None:
        """Read the masking key when the current frame is masked."""
        self.mask_value = self.recv_strict(4) if self.has_mask() else ""

    def recv_frame(self) -> ABNF:
        """Read bytes from the socket until a complete frame is decoded."""
        with self.lock:
            # Header
    ...
    def recv_strict(self, bufsize: int) -> bytes:
        """Read exactly ``bufsize`` bytes from the underlying recv function."""
...
class continuous_frame:
    ...
    def validate(self, frame: ABNF) -> None:
        """Validate that a continuation frame arrives in a legal sequence."""
    ...
    def add(self, frame: ABNF) -> None:
        """Append a fragment to the buffered logical message."""
        if self.cont_data:
    ...
    def is_fire(self, frame: ABNF) -> Union[bool, int]:
        """Return whether the buffered message should be emitted now."""
        return frame.fin or self.fire_cont_frame

    def extract(self, frame: ABNF) -> tuple:
        """Finalize the buffered message and return its opcode with frame."""
        data = self.cont_data
        self.cont_data = None
        frame.data = data[1]
        if (
