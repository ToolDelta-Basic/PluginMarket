...
def setdefaulttimeout(timeout: Union[int, float, None]) -> None:
    ...
    """
    _DEFAULT_TIMEOUT["value"] = timeout
...
def recv(sock: socket.socket, bufsize: int) -> bytes:
    ...
    def _recv():
        """Retry reads when the socket temporarily blocks."""
        try:
...
def recv_line(sock: socket.socket) -> bytes:
    """Receive bytes until a newline terminator is encountered."""
    line = []
    while True:
        c = recv(sock, 1)
        line.append(c)
...
def send(sock: socket.socket, data: Union[bytes, str]) -> int:
    """Send data through the socket with timeout and retry handling."""
    ...
    def _send():
        """Retry sends when the socket temporarily blocks."""
        try:
