...
def _is_ip_address(addr: str) -> bool:
    """Return whether ``addr`` is a valid IPv4 address."""
    try:
...
def _is_subnet_address(hostname: str) -> bool:
    """Return whether ``hostname`` looks like an IPv4 subnet expression."""
    try:
...
def _is_address_in_network(ip: str, net: str) -> bool:
    """Return whether the IPv4 address ``ip`` belongs to subnet ``net``."""
...
def get_proxy_info(
    ...
    _proxy_type: str = "http",
) -> tuple:
    """Return proxy host, port, and auth information for the target host.
