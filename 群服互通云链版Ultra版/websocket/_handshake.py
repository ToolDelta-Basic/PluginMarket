...
def _pack_hostname(hostname: str) -> str:
    """Wrap IPv6 hostnames in brackets for HTTP header serialization."""
    # IPv6 address
    if ":" in hostname:
        return f"[{hostname}]"
    return hostname
...
def _get_handshake_headers(
    ...
    client_cookie = options.get("cookie")
    ...
    if status not in success_statuses:
        ...
        if content_len:
            response_body = sock.recv(
                int(content_len)
            )  # Read the HTTP error response body for the raised exception.
        else:
            response_body = None
        raise WebSocketBadStatusException(
            (
                f"Handshake status {status} {status_message}"
                f" -+-+- {resp_headers} -+-+- {response_body}"
            ),
            status,
            status_message,
            resp_headers,
...
def _validate(headers, key: str, subprotocols) -> tuple:
    """Validate required handshake response headers and subprotocols."""
    subproto = None
    ...
    digest = _create_sec_websocket_accept_digest(value)
...
def _create_sec_websocket_key() -> str:
    """Return a freshly generated ``Sec-WebSocket-Key`` header value."""
