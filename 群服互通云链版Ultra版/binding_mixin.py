...
class QQLinkerBindingMixin:
    ...
    def read_binding_data(self) -> dict[str, dict[str, Any]]:
        ...
        try:
            with open(self.binding_data_path, encoding="utf-8") as file:
                raw = json.load(file)
        except Exception:
            raw = {}
    ...
    def api_get_bound_players_by_qq(
        ...
        return [
            ...
            for xuid in data["qq_to_xuids"].get(
                self._binding_qq_key(qqid),
                [],
            )
        ]
    ...
    def api_start_binding_request(
        ...
        unavailable_message = getattr(
            self,
            "_qqbot_real_qq_unavailable_message",
            lambda *_args: None,
        )(group_id, int(qq_key), "binding")
    ...
    def send_private_msg(
        ...
        user_openid = getattr(self, "_qqbot_user_openids", {}).get(qqid, "")
        if (
            getattr(self, "qqbot_channel_enabled", lambda: False)()
            and qqbot_client is not None
            and user_openid
        ):
            try:
                qqbot_client.send_private_msg(user_openid, msg)
                sent = True
            except Exception as error:
                self._print_cloud_status(
                    "群服互通 官机私信",
        ...
        if not sent:
            self._print_cloud_status(
                "群服互通 私信通道",
                "忽略发送",
                ["没有可用私信通道或缺少用户 OpenID", f"目标身份: {qqid}"],
                level="warn",
            )
        return sent
