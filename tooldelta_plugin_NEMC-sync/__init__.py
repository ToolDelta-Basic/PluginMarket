...
from tooldelta import Chat, Config, Player, Plugin, plugin_entry, utils, cfg
...
class NEMCMessageSync(Plugin):
    ...
    def _send_command_result(self, output: str):
        self._send_json({"s": "ok", "type": "cmd-result", "ouput": output})
    ...
    def set_server_id(self):
        if isinstance(self.frame.launcher, FrameFateArk) or isinstance(self.frame.launcher, FrameNeOmgAccessPoint):
            self.config["服务器ID"] = self.frame.launcher.serverNumber
        elif isinstance(self.frame.launcher, FrameNeOmgAccessPoint):
            if self.frame.launcher.serverNumber is None:
                pass
            else:
                self.config["服务器ID"] = self.frame.launcher.serverNumber
