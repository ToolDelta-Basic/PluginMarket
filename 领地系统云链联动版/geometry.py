...
def sphere_intersects_box(center: Position, radius: float, min_pos: Position, max_pos: Position) -> bool:
    distance_sq = 0.0
    for i in range(3):
...
def boxes_intersect(min_a: Position, max_a: Position, min_b: Position, max_b: Position) -> bool:
...
def bounds_from_center_size(center: Position, size: Tuple[int, int, int]) -> Bounds:
...
def distance_to_land(pos: Position, land: LandData) -> float:
    ...
    return math.sqrt((x - cx) ** 2 + (y - cy) ** 2 + (z - cz) ** 2) - land.radius
...
def land_overlaps_candidate(
    ...
    if shape == "方形":
        candidate_bounds = bounds_from_center_size(center, LandData.normalize_size(size, radius))
        if land.is_box():
            return boxes_intersect(candidate_bounds[0], candidate_bounds[1], *land.get_bounds())
        return sphere_intersects_box(land.center, land.radius, candidate_bounds[0], candidate_bounds[1])
    if land.is_box():
        min_pos, max_pos = land.get_bounds()
        return sphere_intersects_box(center, radius, min_pos, max_pos)
    lx, ly, lz = land.center
    return (
        math.sqrt((center[0] - lx) ** 2 + (center[1] - ly) ** 2 + (center[2] - lz) ** 2)
        <= radius + land.radius
    )
