...
class TpaRequest:
    def __init__(self, sender: Player, target: str, mode="tpa"):
        ...
        if mode == "tpa":
            entry.game_ctrl.say_to(self.target, f"§e{self.sender.name}§f希望传送到你这里.")
        else:
            entry.game_ctrl.say_to(self.target, f"§e{self.sender.name}§f希望你传送到他那里.")
    ...
    def deny(self):
        self.sender.show( f"§6互传请求已被§f{self.target}§6拒绝.")
        entry.game_ctrl.say_to(self.target, f"§a已拒绝§f{self.sender.name}§a的互传请求")
        self.delete()

    def reject(self):
        self.sender.show( f"§6发往§f{self.target}§6的互传请求已被撤销."
        )
        entry.game_ctrl.say_to(
            self.target, f"§6从§f{self.sender.name}§a发来的互传请求已被撤销."
        )
        self.delete()

    def timeout(self):
        self.sender.show( f"发往§e{self.target}§6的请求因超时被取消.")
        entry.game_ctrl.say_to(
            self.target, f"§e{self.sender.name}§6发来的请求已超时， 被自动取消."
        )
        self.delete()

    def tp(self):
        if self.mode == "tpa":
            entry.game_ctrl.sendcmd(f"tp {self.sender.getSelector()} {self.target}")
        else:
            entry.game_ctrl.sendcmd(f"tp {self.target} {self.sender.getSelector()}")
